...
def test_roadmap_skill_is_meta_and_delegation_first() -> None:
    ...
    assert "phase: meta" in skill
    assert "phase: roadmap" not in skill
    assert "permitted_tools: [Read, Grep, Glob, Bash]" in skill
...
def test_roadmap_skill_stops_at_plan_and_owns_no_implementation_path() -> None:
    ...
    assert "**NEVER** modify production implementation from Roadmap." in skill
    assert "**ALWAYS** stop at `/qor-plan` handoff." in skill
    assert "implementation-task nodes" in skill
