...
### Step 5.5: Continuity outcome routing (Phase 216 wiring; GH #285)

When a `continuity_outcome` is present, route on it and do not collapse it into
`status`:

- `verified` -- continue on the existing governed path.
- `rejected` -- route to `/qor-remediate` against the defect the receipt names
  (product, governance, security, claim, or authority).
- `inconclusive` -- repair or change the evidence environment. Do NOT assert
  product failure; the gate ran and the environment denied it a conclusion.

Revision movement after verification invalidates the receipt: require
re-verification rather than carrying the prior outcome forward. `inconclusive`
is not `skip`; see `qor/references/doctrine-execution-continuity.md`.
