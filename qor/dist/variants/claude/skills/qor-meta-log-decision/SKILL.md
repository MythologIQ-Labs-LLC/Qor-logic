...
Concrete invocation examples (Architecture L2, Security L3, Scope Change L2) are documented in `qor/skills/meta/qor-meta-log-decision/references/example-decision-entries.md` per the progressive-disclosure doctrine (`SG-SkillCorpusGrowth-A`). The reference file shows command, output, and contextual narration for each tier.
