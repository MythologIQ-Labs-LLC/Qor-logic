...
Verify no live skill carries retired named-model admission metadata and that fabrication-risk skills keep their negative-constraints guard. Non-blocking WARN; operator decides how to proceed.

```bash
qor-logic scripts model_pinning_lint --repo-root . || true
```

Phase 240 retired named-model admission: the lint now walks `qor/skills/**/SKILL.md` for the retired `model_compatibility` / `min_model_capability` frontmatter (their presence is the warning) and for fabrication-risk skills missing the `doctrine-negative-constraints` guard pointer; execution-context capability evidence is authoritative per `qor/scripts/execution_context.py`. Per `qor/references/doctrine-ai-rmf.md` §MANAGE-3.1.
