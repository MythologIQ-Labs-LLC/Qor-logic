...
qor-logic scripts plan_enumeration_lint --plan "$PLAN_PATH" --repo-root . || true
...
### Step 0.7: Spec-delta pre-pass (Phase 192; GH #277)

Lint declared deltas (`spec_lint --delta`); failures VETO as `specification-drift`. Protocol: `references/spec-delta-pre-pass.md`.
...
Adversarial mode contract (input/output schemas) lives in `qor/skills/governance/qor-audit/references/adversarial-mode.md`. Every dispatched reviewer is bound by the Reviewer toolset declaration contract there (Phase 228; GH #342): it declares its toolset in its first response, and the Judge may not pin a verification outside that set.
...
`version_applicability.validate` on the active plan before PASS: a release-class target that cannot exceed the current tag -> **VETO (`specification-drift`)**; `change_class: governance` is version-not-applicable. Procedure: `references/version-applicability-pass.md`.
...
#### Execution-Continuity Pass (Phase 216 wiring; GH #285)

Applies when the plan declares `execution_continuity`. Verify:

- [ ] Provider budget/context/capacity exhaustion is resumable when a valid
      checkpoint exists -- not classified as product failure or auto-escalation
- [ ] Absence, malformation, tampering, revision mismatch, live-writer conflict,
      claim conflict, and actor-authority mismatch each fail closed
- [ ] Environment unavailability yields `inconclusive`, never a fabricated pass
      or a product rejection
- [ ] Implementation self-report cannot satisfy independent verification
- [ ] The verification target is the exact revision under review (equality, not
      ancestry -- `intent_lock` semantics must not be inherited)
- [ ] The plan consumes the upstream contract by compatibility version and
      duplicates no field definition

**Any violation -> VETO with `continuity-unsound` category.**

**Required next action:** Governor: amend plan text, re-run `/qor-audit`. Rule
order and rationale: `qor/references/doctrine-execution-continuity.md`.
