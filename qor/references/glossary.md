...
```yaml
term: range-pin lower-bound check
definition: 'Phase 107 extension to the dependency-admission cooling-period lint: range-pin specifiers `>=X.Y.Z` and `~=X.Y.Z` in `[project] dependencies` / `[project.optional-dependencies]` are walked by `parse_pyproject_range_pins(text)` which returns LockfileEntry instances carrying the lower-bound version as `version`. The lint applies the 14-day threshold to those lower-bound versions because pip''s installer could resolve to that version on a fresh install. `<`, `!=`, and unbounded specifiers are skipped. Pairs with the Phase 106 exact-pin parser to provide complete cooling-period coverage of pyproject dependency declarations.'
home: qor/references/doctrine-dependency-admission.md
referenced_by:
  - qor/scripts/_dep_admit_common.py
  - qor/scripts/dependency_admission_lint.py
...
```yaml
term: SBOM hash-pinned lockfile
definition: 'Phase 107 closure of Phase 102 carry-forward: `requirements-sbom.in` declares the SBOM tool dependency (`cyclonedx-bom`); `requirements-sbom.txt` is the pip-compile --generate-hashes autogenerated lockfile that hash-pins cyclonedx-bom and its full transitive dependency set (~390 lines; lxml dominates). The release.yml build job installs via `pip install --require-hashes -r requirements-sbom.txt`. The lockfile is Razor-exempt per the Phase 107 plan''s explicit narrow exemption block (autogenerated; size tracks dependency graph, not authoring complexity).'
home: qor/references/doctrine-dependency-admission.md
referenced_by:
...
```yaml
term: override rollup issue
definition: 'Phase 107 closure of Phase 105 calendar/GH-issue carry-forward: the `dep_admit_override_tracker` script gains `--emit-issue` which builds a single rollup GitHub issue body listing all due (>=30 days old) override entries from META_LEDGER. Invoked via `gh issue create --label dep-admit-override-review`. One rollup issue per invocation (anti-spam); operator runs on their own cadence. Silent (returns None) when no due entries.'
home: qor/references/doctrine-dependency-admission.md
referenced_by:
  - qor/scripts/dep_admit_override_tracker.py
  - qor/references/doctrine-dependency-admission.md
...
```yaml
term: CODEOWNERS operational mode
definition: 'Documented operational mode for the `.github/CODEOWNERS` file. Phase 107 closes the Phase 101+102 carry-forward "broaden CODEOWNERS reviewer pool" by documenting that the current solo-owner state (`@Knapp-Kevin` for all security-critical files) is a deliberate operational mode for solo-maintained governance repositories, not an interim placeholder. The doctrine `qor/references/doctrine-governance-enforcement.md` §6.1 names four expansion triggers (second maintainer joins, federation, compliance audit, operator-initiated team formalization); until one fires, the solo-owner state is the chosen mode.'
home: qor/references/doctrine-governance-enforcement.md
referenced_by:
  - .github/CODEOWNERS
  - qor/references/doctrine-governance-enforcement.md
