...
`requirements-sbom.txt` (the hash-pinned SBOM toolchain lockfile
installed by the release build), or direct deps in `pyproject.toml`).
...
At Phase 103 the check was manual. Phase 105 added
`qor/scripts/dependency_admission_lint.py`, and Phase 107 made it
hard-fail in `.github/workflows/pr-dependency-review.yml`.
...
**Phase 298 lockfile coverage**: the CI admission step runs once per governed root lockfile (`requirements-release.txt`, `requirements-sbom.txt`) via `--lockfile`, each step hard-fail; `requirements-*.in`, `requirements-*.txt` and `pyproject.toml` changes all trigger the workflow. The 14-day threshold and the override procedure are unchanged. Declared residual: the CLI entry point does not pass `pyproject.toml` to the pyproject pin walk, so pyproject pins are not examined in CI.
...
threshold is operative in `pr-dependency-review.yml`, for the governed
root lockfiles only.
