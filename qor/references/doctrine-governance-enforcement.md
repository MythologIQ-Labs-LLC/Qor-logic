...
Feature and breaking phases MUST carry current README literal-count badges (Tests, Ledger, Skills, Agents, Doctrines) and SYSTEM_STATE header fields. Since Phase 164 (research entry #378) these are GENERATED, not hand-edited: `/qor-substantiate` Step 7.7.5 runs `qor.scripts.seal_artifacts --write` to regenerate them from truth (via the `qor/scripts/badge_currency.py` counters) and then `seal_artifacts --check`; ABORTs seal on mismatch. The pair sits after Step 7.7 has verified the appended SESSION SEAL entry exists, because the ledger badge counts ledger entries: generating it at Step 6, as this doctrine specified before Phase 224 (GH #334), graded the artifact against the state that produced it and shipped a badge one behind on every seal. Hotfix is exempt (matches `_RELEASE_CLASSES` semantics). The same `--check` runs as a CI step on sealed state.

Locked at the test layer by `tests/test_seal_artifacts.py` (behavioral tests of the generators against synthetic fixtures) and `tests/test_substantiate_seal_artifacts_wiring.py` (Step 7.7.5 wiring regression lock). The pre-164 live-equality class (5 badge tests + 2 SYSTEM_STATE header tests + 6 prose-wiring tests) is retired: it asserted generated-artifact state against moving truth and broke on nearly every seal (phases 121/122/123/140).

The counted topology is declarable (Phase 206; GH #293). `qor/scripts/badge_layout.py` carries it as one immutable value whose field defaults are this repository's `qor/skills`, `qor/agents`, and `qor/references` roots, so an adopter that declares nothing keeps the historical behavior. Both the writer and the checker consume the same declared value, supplied either as the single `layout` keyword or via the `--skills-root` / `--skills-pattern` / `--agents-root` / `--agents-pattern` / `--doctrines-root` / `--doctrines-pattern` flags that `seal_artifacts` and `badge_currency` share. A configured root that does not resolve to a directory is a resolution error that names the root and ABORTs, never a synthetic zero: an adopter whose topology differs is told so, rather than being handed a false empty count. Roots and matched files are confined to both the repository and the declared root; traversing patterns, absolute patterns, and matched symlinks are rejected. A root that resolves and is genuinely empty still counts zero -- that case is declared, not unresolved. Locked at the test layer by `tests/test_badge_layout_resolution.py`.

The declared topology is reachable from the governed flow (Phase 210). Each of the six keys resolves independently as **CLI flag > `.qorlogic/config.json` `layout.<key>` > `qor/`-rooted default**, so a repository declares only what differs and the rest keeps its default. Resolution happens inside `layout_from_args`, which both the checker and the writer already call with the `--repo-root` the governed invocations already pass -- no skill or workflow edit is needed to reach it. Flag defaults are `None` so an unset flag is distinguishable from one set to the default; while they were the real layout values a flag always arrived populated and no configuration could ever have applied. Every malformed shape (absent file, invalid JSON, non-object document or section, wrong-typed, empty, or blank value) degrades to the default and never raises: a broken operator config must not break a gate, and it must not turn a failing seal into a passing one. Declaring a root is not trusting it -- config supplies candidates, and repository containment, pattern-traversal rejection, and symlink rejection all still apply, asserted by negatives that enter through the configuration channel rather than inheriting the flag-channel tests. Locked by `tests/test_badge_layout_config.py`.
...
2. Writes `.qor/session/<sid>/escalation_suppressed` with the current timestamp. `cycle_count_escalator._suppression_active` checks this marker against an anchor timestamp, and the two escalation modes anchor differently, so the marker file is shared but the suppression it produces is not (Phase 266, GH #447):
   - `check` (consecutive streak) anchors on the OLDEST audit of the current run and compares strictly. Its run resets on a PASS, a signature change or an implement break, so the anchor advances past the marker and the escalation re-fires. Suppression here lasts until the next reset, **not** the remainder of the session.
   - `check_session_total` (cumulative per-signature) anchors on the K-window floor, the oldest of the most recent `ESCALATION_THRESHOLD` occurrences of that signature, and compares inclusively so a decline sharing a whole second with the anchor still lands. Its counter never resets, so the window is what bounds the silence: a decline holds until that signature recurs `ESCALATION_THRESHOLD` further times. Suppressed signatures are filtered before a winner is selected, so declining one never masks another.
