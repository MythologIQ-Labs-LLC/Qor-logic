...
**Originating recurrence**: a sibling consumer-workspace session 2026-05-08T1610-21dfe5, 3-VETO cycle on the same drift signature across 2 plans (META_LEDGER #200, #201, #203 in the consumer workspace).

**Countermeasure**: `qor/scripts/plan_text_consistency_lint.py` (sealed at Phase 55 from a sibling consumer workspace; wired upstream at Phase 67 GH #42) detects same-operation drift via stem extraction + grouping. Wired into `/qor-plan` Step 5 review checklist (operator self-check) and `/qor-audit` Step 0.6 (WARN-only pre-audit lint). Tolerance is zero: any drift in commands, dependencies, or paths returns exit 1 with the divergent sites named.
...
**Originating recurrence**: an external QA exemplar's app `attribution-12g` plan, audit cycle 2026-05-13, iter-1 -> iter-3 audit chain. Iter-1 included a citation to a sealed migration column that did not match the actual schema; iter-2 audited the diff against iter-1 and did not re-grep the unchanged citation; iter-3 surfaced the mismatch via independent reviewer (SG-AuthorAuditMomentum-A), but the citation had already shaped two iterations of plan structure.

**Countermeasure P1** (`/qor-plan` Step 2 Infrastructure Citation Inventory): every LD citing sealed infrastructure MUST carry a paired grep-evidence statement of the form `git show <sealed-ref>:<path> | grep -nE '<pattern>' -> NN:<exact observed text>` (the `grep -n` output line; the `NN:` prefix is what makes the statement mechanically resolvable, and a statement without it is presence-only). Citations without paired evidence are Open Questions, not LDs, and block submission to `/qor-audit`. **Enforced (Phase 125; GH #152)** by `qor.scripts.plan_grep_lint.check_citation_evidence`, which runs at `/qor-audit` Step 0.6 (WARN-only pre-audit layer). **Phase 223 (GH #330) tightened it from per-block to per-citation for the checkable kind.** **Phase 225 (GH #336) made the canonical statement first-class: a parsed grep-n statement is adjudicated on its own account (nothing needs to demand it), findings carry bare `path:line` citation keys, and the reported count is the per-block union of parsed statements and demanded citations.** Truth-checked kinds: `file:line`, `grep-n evidence`. Presence-only kinds: `migration filename`, `bare git show ref-path`. A `file:line` citation must carry its own statement naming the same path and line, and that statement must reproduce -- three findings distinguish the ways it can fail: `unpaired-citation` (no statement of its own), `evidence-not-reproducible` (the cited line does not hold the quoted text), and `evidence-unresolvable` (the path will not resolve at the cited revision, an environment that cannot answer rather than an answer that is wrong). The presence-only kinds carry no line to verify and keep the block-level rule. The demand set is deduplicated by `(path, line)`, so a restating table does not inflate the reported count. LD-region-scoped, so plans not using the discipline produce zero findings; note that artifacts with no Locked-Decision heading -- audit reports, remediation proposals -- are outside the scanned region entirely, which is a known gap rather than a satisfied contract. The binding VETO remains P2 below.
...
**V2 stopgap** (Phase 91 wiring; GH #85): `qor-logic verify-ledger --tolerate-known-grandfathered` accepts chain-math failures iff the failing entries' `previous_hash` appears in the ledger's duplicate-`previous_hash` set AND the failing entry numbers are `<= --grandfather-cutoff` (default 207, matching `check_previous_hash_uniqueness`'s `min_entry_num`). Read-only verifier semantics — the ledger is not modified, so no operator-authorization protocol is needed in V1. Lets consumer workspaces (e.g., an external QA exemplar's app per GH #85) ship clean `verify-ledger` gates immediately without rewriting past entries. The flag is OFF by default; the strict verifier remains the canonical gate unless the operator opts in. Tolerated failures emit `DISCLOSED_GRANDFATHERED Entry #N: tolerated SG-ConcurrentLedgerRace-A residual` on stdout (not stderr), do not contribute to the error count, and do NOT propagate TAINTED to downstream entries. Implementation: `qor.scripts.ledger_hash.find_grandfathered_entries(ledger_md, cutoff=207) -> frozenset[int]` enumerates the residual set; `verify(..., tolerate_known_grandfathered=True, grandfather_cutoff=207)` honors the flag.

**V2 real fix** (Phase 119 wiring; GH #148): `qor-logic reconcile propose --ledger <path>` detects the duplicate-`previous_hash` residual and writes a pending proposal artifact (read-only); `qor-logic reconcile authorize --proposal <path> --ledger <path>` appends a forward-only **RECONCILIATION** entry that enumerates the attested residual on a `**Reconciled Entries**: #a, #b, ...` line. `verify()` collects those attested numbers, intersects them with the genuine duplicate-`previous_hash` member set (a security gate — an attestation cannot launder content tampering of a unique-`previous_hash` entry), and reports `DISCLOSED_RECONCILED Entry #N: attested by RECONCILIATION entry` for the attested residual **without** the `--tolerate-known-grandfathered` flag; non-attested chain failures still FAIL. The two-stage propose->authorize contract mirrors Phase 36 B19 (the explicit `--proposal <path>` arg is the sole operator-authorization signal). **Forward-only is preserved**: the RECONCILIATION entry is appended, never rewriting or renumbering sealed entries #1-#207 — the "Forbidden interpretation" clause above remains binding. The `--tolerate-known-grandfathered` flag is retained as the ephemeral stopgap; the RECONCILIATION entry is its permanent, in-chain successor. Implementation: `qor/scripts/reconcile.py` (`detect_residual`, `build_proposal`, `append_reconciliation_entry`) + `qor/cli_handlers/reconcile.py`.
...
**Originating recurrence**: a sibling product's v5.1.0 Install QorLogic Skills card -- `install-skills-card.js` sets `progressBar.style.width = '0%'` at start; operator click fires `POST /api/actions/scaffold-skills`; backing pipeline runs 5 sequential phases (Python probe -> pip install -> per-host `qorlogic install` -> provenance verification -> hub refresh) over 20-60 seconds; modal shows frozen 0% throughout; `skills.install.progress` WebSocket events exist but the modal does not subscribe; bar jumps to 100% on completion. Two prior qor-audit PASS cycles (the sibling product's META_LEDGER #361 + #362) missed the defect because the legacy Ghost UI Pass checked only handler presence, not live-feedback fidelity.
...
**Cross-reference**: Issue #58; the sibling product's META_LEDGER #360 (debug report for the misattribution caused by the same async-vs-UI timing gap), #361/#362 (the two PASS audits that missed the defect), #366 (audit-VETO of the in-repo amendment that filed this upstream issue). Future V2: mechanical `qor/scripts/plan_live_progress_lint.py` heuristic at Step 0.6 pre-audit lint surface (deferred to follow-on phase pending V1 prose adoption).
...
**Cross-reference**: Issue #47; the sibling consumer workspace's META_LEDGER #209 (sealed defect), the sibling workspace's commit `0999e47` (operator fix), the sibling workspace's PR #82 merge review (detection event); doctrinally adjacent to Issue #44 (Wave 2 self-targeting remediation plan check -- shared blind-spot class). Future V2: mechanical `qor/scripts/plan_filter_stage_lint.py` AST helper across Rust/Python/TypeScript pipeline shapes (deferred to follow-on phase pending V1 prose adoption).
...
**Cross-reference**: SG-007 (predecessor narrative); Issue #44, Issue #50, Issue #82; SG-AffectedFilesContract-A (Phase 110; the cascade family whose Option-B auto-dispatch this entry's risk-score mechanism was extended to cover); a sibling governance repository + a sibling consumer workspace empirical evidence.
...
**Originating recurrence**: a sibling consumer workspace's Phase 399 (consumer-ledger META_LEDGER #435 / #436 / #437; SHADOW_GENOME 2026-05-27 iter-1 + iter-2). Four same-family defects across one audit cycle:
...
**V2 enforcer SHIPPED** (Phase 129; GH #153 + #154): the `/qor-substantiate` Step 4.6.8 merge-velocity check now **fail-closes** — `exceeded` ABORTs the seal (the `|| true` wrap is removed), imposing the throttle instead of merely reporting it. The operator escape is `merge_velocity_check --override`, which emits a logged `gate_override` shadow event (`details.gate = merge_velocity_check`) and passes — the explicit operator-authorized path the #153 AC requires. The forward companion `workspace_fragility_check` (#154) regained the dropped `stabilization_capacity` + `shared_surface_risk` fields and the `branch_only` recommended action (returned when `shared_surface_risk == "high"`), and now runs in `/qor-plan` (Step 0.4 stabilization-capacity checkpoint) and `/qor-implement` (Step 0.6 scope-boundary warning) in addition to `/qor-audit` Step 0.6 — WARN-only at those three sites; only the substantiate merge-velocity check enforces. Still reserved: GitHub-API integration (failing-e2e tail-check; test-matrix expansion; cross-PR repair-density clustering).
...
**V2 counterweight SHIPPED** (Phase 132; GH #162): two reserved directions now exist. (1) `qor.scripts.progressive_disclosure_lint` auto-suggests progressive-disclosure refactor candidates — it flags any SKILL.md heading section whose inline prose (fenced code excluded) exceeds a per-section budget while not pointing to a `references/` file (escape `<!-- qor:inline-prose-ok -->`). (2) `qor.scripts.corpus_consolidation_report` is the periodic consolidation cadence: it aggregates total corpus bytes + `skill_size_budget_lint` findings + the progressive-disclosure candidates into a ranked worklist (EXCEEDED skills, then WARN, then extractable sections) and is wired into `/qor-process-review-cycle` Phase 4 as the recurring corpus-weight sweep. Both are advisory (they suggest; the operator consolidates). **Still reserved**: context-load measurement per phase (per-skill reference fan-out) and growth-rate analysis from git history (the GH #92 measurement table generalized into a script).
...
**V2 shipped** (Phase 99 wiring; GH #108 full close): new `qor.scripts.runtime_contract_walk` walks the import graph one hop in each direction for each Python module path cited in a plan being audited. Forward walk: cited module's own imports all parse via subprocess `python -c "import X"`. Backward walk: at least one production (non-test, non-scratch, non-doc) caller of the cited module exists and parses cleanly with `ast.parse`. Wired into `/qor-audit` Step 3 Infrastructure Alignment Pass as a NEW sub-check after the existing grep-verify block. WARN-only ramp in V2 (CLI exits 0 by default; `--exit-on-any` opts into hard fail; the audit-site invocation uses `|| true` wrap) because no Phase 96 V1 operator evidence has accumulated in the same-session cluster. A future V2-of-V2 phase will gather V1 operator-evidence on false-positive rate, tune walk thresholds and skip rules, remove the `|| true` wrap at the audit-site, and convert the walk to a hard VETO with the `runtime-contract-mismatch` category. Multi-language walk extension (Rust/TS/JS) remains deferred to a separate future phase. See `qor/references/audit-runtime-contract-walk.md` for the detailed two-direction protocol.
...
## SG-HarnessSignalDrift-A — harness-provided signal leaks across the skill boundary (Phase 138; confirmed Phase 131)

**Pattern**: a harness-provided per-skill signal — the `QOR_SKILL_ACTIVE` environment variable that one skill sets to label its own gate-artifact provenance — persists across the skill-boundary context into a *different* skill's execution and mislabels that skill's artifacts. The signal is ambient (process env) rather than passed explicitly, so it drifts: a child skill invoked within the same process/env inherits the parent's `QOR_SKILL_ACTIVE` and stamps its `write_gate_artifact` provenance with the wrong skill name.

**Originating recurrence**: GH #138. `QOR_SKILL_ACTIVE` set during one skill's gate-artifact write leaked into a subsequently-invoked skill, so the second skill's provenance manifest recorded the first skill's name. The ambient env signal had no scoping discipline at the skill boundary.

**Countermeasure** (Phase 138 wiring; GH #138; SHIPPED in PR #144, v0.78.0): the env signal is scoped with a context-manager that restores the prior value on exit, `write_gate_artifact` gained an explicit `skill=` parameter so provenance is passed-not-inherited, an active-phase reporter surfaces the resolved skill, and a restore-on-exit regression test locks the scoping. Explicit-parameter provenance is preferred over ambient-env inheritance at every skill boundary.

**Moot sub-finding** (confirmed Phase 131; GH #165): `shadow_process.append_event` was listed at #138 as a candidate to thread a `skill` param through, but it is genuinely moot — `append_event` consumes only the caller-supplied `event["skill"]` data field and never reads `QOR_SKILL_ACTIVE` or any environment variable, so the harness-signal leak does not flow through it. No change to `append_event` is required; `tests/test_harness_signal_drift.py::test_append_event_ignores_qor_skill_active_env` proves the non-consumption by behavior, and `::test_append_event_source_has_no_env_read` guards against regression.

**Cross-reference**: GH #138 (originating fix) + GH #165 (residue closure). Doctrinally adjacent to `SG-HalfSealedClaim-A` (Phase 75 — host/archetype signals not faithfully reflected in the seal). Same root family: a provenance/identity signal that must be explicitly scoped rather than ambiently inherited.
