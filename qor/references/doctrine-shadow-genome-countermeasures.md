...
**Cross-reference**: GH #89; doctrinally adjacent to `SG-DoDImplicit-A` (Phase 92 — ceremonial gates pass while substantive verification is absent) and `SG-CICoverageDrift-A` (Phase 89 — plan ci_commands not reconciled against actual CI workflows). Same root family: governance gates that detect a class of process pressure the standard PASS/VETO sequence cannot see.
