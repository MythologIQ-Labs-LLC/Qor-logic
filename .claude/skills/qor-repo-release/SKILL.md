...
#### Local CI Mirror (MANDATORY — HARD GATE)

Before pushing ANY code to CI, run the exact CI commands locally to catch issues before burning CI minutes:
...
**INTERDICTION**: If ANY local CI mirror command fails:
```
ABORT
Report: "Local CI mirror failed: [command] returned errors.
         Fix ALL issues locally, batch into one commit, then re-run pre-flight.
         Do NOT push individual fixes to CI."
```

#### CI Cost Estimate

Before triggering the release pipeline, calculate expected CI cost:
- Estimate number of CI jobs that will run (matrix: platforms × steps)
- Estimate total CI minutes
- If estimated cost exceeds 50% of remaining monthly budget, WARN the user

#### Single-Push Policy

**Rule**: All fixes must be batched into a single commit before pushing. The fix-push-wait anti-pattern (pushing one fix, waiting for CI, pushing another) is forbidden.

#### Release Workflow Validation (Dry-Run)

Before triggering the actual release pipeline:
1. Verify the release workflow YAML syntax is valid
2. Verify all referenced tools/packages exist at their expected paths
3. Verify platform-specific requirements (icon sizes, version formats, runner availability)
4. If `act` (local GitHub Actions runner) is available, run the release workflow locally
...
**INTERDICTION**: Tags must NEVER be created before:
1. ALL PR checks are green
2. The PR has been merged to the target branch
3. The release workflow has been validated locally (Step 2 dry-run)

If checks are not green:
```
ABORT
Report: "Cannot tag — PR checks are not all green. Wait for CI to pass before tagging.
         Tagging before CI passes triggers broken release builds that waste CI minutes."
```
...
- **NEVER** tag before all PR checks are green and PR is merged — tag-after-merge only
- **NEVER** push to CI without passing local CI mirror first (Step 2)
- **NEVER** push individual fix commits — batch ALL fixes into one commit (single-push policy)
- **NEVER** force-push to shared branches (main, staging) without GR-2 coordination protocol
- **NEVER** push secrets or leave them in history — rotate, rewrite, then gitignore (GR-1)
...
- **ALWAYS** run local CI mirror before any push (catch issues locally, not on CI)
- **ALWAYS** estimate CI cost before triggering release pipeline
- **ALWAYS** validate release workflow locally before tagging (dry-run)
