...
Launch the `qor-fixer` agent (use `subagent_type: "qor-fixer"`) with the problem description. The fixer runs all four layers (Dijkstra, Hamming/Shannon, Turing/Hopper, Zeller) focused on the REPORTED symptoms. It identifies root causes and proposes fixes.

**Before applying fixes**: If the root cause is a missing or incorrect implementation, write a failing test that reproduces the issue FIRST. The test must fail with the current code and pass with the proposed fix. This proves the fix is correct and provides regression protection.
...
- **NEVER** push individual fix commits — batch all fixes from the residual sweep into one commit
...
- **ALWAYS** use `subagent_type: "qor-fixer"` (not `ultimate-debugger`)
- **ALWAYS** write a failing test before applying any fix (proves the fix and prevents regression)
