...
#### Test Audit
```
Glob: tests/**/*.test.{ts,tsx,js,rs}
Glob: src/**/*.{ts,tsx,js,rs}
Read: Test files
```

**TDD Evidence Check (HARD GATE)**:

For every new implementation file (`.rs`, `.ts`, `.tsx`, `.js`) introduced since the last seal:

1. Verify a corresponding test file exists
2. Verify the test was created BEFORE the implementation (check git log timestamps)
3. If ANY implementation file exists WITHOUT a corresponding test:

```
FAIL: "TDD Evidence Gap — [file] has no test coverage.
       Implementation without tests violates the TDD mandate.
       Either add tests or remove the implementation."
```

**IF TDD Evidence Check FAILS**:
- Do NOT proceed to sealing
- Report every file missing test coverage
- Hand back to `/qor-implement` to add tests
...
- **NEVER** seal with TDD Evidence Gap — every implementation file must have a corresponding test
...
- **NEVER** seal when branch lineage is broken (merge base is ancient — rebase first per GR-3)
- **NEVER** push secrets or leave them in history (GR-1)
- **NEVER** tag before merge — all PR checks must be green first
- **NEVER** push individual fix commits (batch all fixes, push once)
- **ALWAYS** validate version before sealing
- **ALWAYS** run CI-equivalent commands locally before push (lint, test with CI flags)
