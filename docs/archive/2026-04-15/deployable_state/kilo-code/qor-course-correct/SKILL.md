...
description: >-
  Process-level drift recovery through collaborative diagnosis. Use when: (1) Multiple consecutive VETOs from /qor-audit, (2) Recurring Shadow Genome patterns (same failure >2 times), (3) User feels stuck or direction has gone wrong, (4) Scope creep detected (implementation diverging from CONCEPT.md), or (5) Significant time elapsed with no /qor-substantiate seal.
metadata:
...
Recover from PROCESS-LEVEL drift — the gap between where you ARE and where CONCEPT.md says you should BE. This skill does NOT fix code bugs (that is /qor-debug and /qor-refactor). It fixes DIRECTION: misaligned scope, design mismatch, complexity spirals, blocked dependencies, and lost orientation.

This skill can amend CONCEPT.md and ARCHITECTURE_PLAN.md directly. It does NOT route through /qor-bootstrap (bootstrap is one-time workspace init only).
...
Run /qor-bootstrap first to initialize the workspace."
```
...
- **Architecture changed** -> Route to `/qor-audit` for re-evaluation
- **Scope reduced** -> Route to `/qor-plan` for revised plan
- **Dependency unblocked** -> Route to `/qor-implement` to continue
- **Direction clarified** -> Route to `/qor-plan` for next milestone
...
- **NEVER** route through /qor-bootstrap (bootstrap is one-time init only)
...
- **ALWAYS** verify branch lineage before proceeding if recovery involves git operations (GR-3)
...
- **Skill Routing**: Hands off to /qor-audit, /qor-plan, or /qor-implement based on recovery outcome
