...
**Upstream Input**: For non-trivial features, the Strategist (`/ql-research`) delivers a research brief (`<legacy-staging>/RESEARCH_BRIEF.md`) that defines *what* and *why*. The Governor translates this into *how* — the architectural blueprint. If no brief exists for a complex feature, recommend `/ql-research` first.
