...
**Snapshot**: 2026-06-10
**Chain Status**: ACTIVE. Phase 160 (hotfix, v0.110.2) was a documentation-currency pass; Phase 161 (hotfix, v0.110.3) closed a `PYTHONHASHSEED`-coupled CI flake; this entry (Phase 162, feature, v0.111.0) implements GH #231 Option 1 -- a WARN-first ledger base-currency gate that linearizes the hash chain at the trunk. (PyPI is a continuous 0.109.0->0.109.5 + 0.110.0->0.110.3 line, now + 0.111.0, as of 2026-06-10.)
**Phase**: Phase 162 (feature; GH #231 Option 1). The META_LEDGER is a linear hash chain carried in a file that lives in a git branch DAG: a branch that seals against a stale `origin/main` tip forks the chain, and git can auto-merge the two appends silently. New `qor.reliability.ledger_base_currency`: `check_base_currency(branch_text, base_text)` flags a branch whose first new-on-branch entry's `previous_hash` != `origin/main`'s tip `chain_hash` (new entries identified by chain-hash set membership, robust to the entry-number reuse #231 is about); `reanchor(base_tip, new_entries)` is a pure fold rebuilding a provisional sub-chain's `previous_hash`/`chain_hash`/`entry_id` onto the live base tip without editing the ledger (content hashes unchanged, so the GOV-01 binding is preserved). Wired WARN-only in CI (`--enforce` reserved for the V2 flip); the post-hoc `check_previous_hash_uniqueness` detector (`SG-ConcurrentLedgerRace-A`) is retained as defense-in-depth. New `doctrine-ledger-concurrency.md` (provisional-until-merge contract; home for `ledger base currency` + `provisional seal entry`). Self-application verified: Phase 162 was branched from the live #373 tip, so its own first new entry (#374) chains from `1ec244e9...` and the WARN-only gate passes on itself. The hash chain's job is tamper-evidence, not coordination -- git provides coordination; this makes `main` the linear source of truth.
