...
**Snapshot**: 2026-06-10
**Chain Status**: ACTIVE. Phase 155 (hotfix, v0.109.3) made `verify()` FAIL a modern (>= #123) entry that lacks canonical hash markup (GAP-GOV-09); Phase 156 (hotfix, v0.109.4) re-verified the COMMITTED seal's content_hash<->plan binding in CI and fixed a CRLF fragility in the GOV-01 binding (GAP-GOV-03); this entry (Phase 157, hotfix, v0.109.5) closes the same CRLF fragility class for the second seal-relevant file hasher.
**Phase**: Phase 157 (hotfix, GAP-GOV-03 follow-on). `hash_guard.hash_file` -- the other seal-relevant file hasher (cited in `/qor-substantiate` Step 6.8 Preparation) -- still hashed raw bytes, so a digest it produced over a text seal artifact would drift on git autocrlf exactly as `content_hash` did pre-156. Because `hash_file` is also a general-purpose / binary hasher, the fix is an OPT-IN `normalize_newlines: bool = False` keyword param: True LF-normalizes before hashing and reports `byte_count` over the hashed bytes; the byte-exact default leaves every existing call unchanged. `intent_lock._hash_file` is deliberately excluded (intra-checkout gate-artifact comparison, no git round-trip). Guidance routed to the substantiate `references/seal-gate-ladder.md`, leaving the at-budget SKILL.md untouched. The remaining GH #210 (Sprint A) item is GAP-GOV-05 (self-asserted provenance; needs a non-forgeable signal). **Recent arc (Phases 146-157, all published to PyPI through v0.109.x):** FEATURE_INDEX backfill (#146); a 6-agent production-gap red-team audit (Entry #353) -> Sprint C session_id path-safety + citation accuracy (#147), Sprint B compliance-enforce conveyance + py.typed (#148, GH #211), README currency from a /qor-document review (#149), Sprint A content_hash<->plan binding + provenance/completeness hardening (#150) + dead-hasher removal (#151) + verify() decompose (#153) + markup-required floor (#155, GAP-GOV-09) + committed-seal re-verify & content_hash CRLF-invariance (#156) + hash_file CRLF-invariant seal-text option (#157, GAP-GOV-03); plus the Shadow Genome producers (#152, GH #213) and Claude Fable 5 (Mythos-class) model-adaptive comms research (Entry #354, deferred). Open backlog: #209 (audit umbrella) + #210 (Sprint A: only GAP-GOV-05 remains); #211/#212/#213/#219 closed; Fable 5 feature deferred. Full per-phase history is authoritative in `docs/META_LEDGER.md` (365 entries; latest Entry #365 -- Phase 157 -- v0.109.5 -- chain `<sealed below>`); SYSTEM_STATE keeps the current-state header, physical map, and a condensed recent-phase bridge rather than restating every phase. The pre-1.0 line is current: the half-measure-closures cluster (GH #147 + #148-#165) is fully closed with real enforcers, and the two largest governance skills remain under the 40 KB skill-size budget (Phases 135/136).
