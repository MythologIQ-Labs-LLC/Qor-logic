...
**Snapshot**: 2026-07-04
**Chain Status**: ACTIVE. Phase 166 (feature, v0.114.0) made SG closure require an executable enforcer; Phase 167 (feature, v0.115.0) completed the autonomous-QA cluster with dry-run rehearsal on every mutating surface; this entry (Phase 168, feature, v0.116.0) makes gate depth scale with declared risk. (PyPI is a continuous 0.109.0->0.111.0 line, + 0.111.1 through 0.115.0 in flight pending `pypi` approvals, as of 2026-07-04.)
**Phase**: Phase 168 (feature; risk-tiered gate depth; research entry #394, closes GH #248). A plan may declare `required_gate_artifacts` (Shape 3, operator-selected): the short chain `[plan, implement, substantiate]` -- omitting ONLY the adversarial audit -- is permitted exclusively for L1-risk hotfix changes (risk routing over affected files via `qor/capabilities/risk.py`), and is never silent (severity-1 `gate_override` shadow event on declaration). Fail-closed at four seams: plan schema (release classes can never omit audit; mandatory-minimum membership), new `qor/scripts/tier_guard.py` at implement-prior resolution (the Phase 59 carve-out shape), and the per-session declared set honored by BOTH `gate_chain_completeness` and `gate_provenance verify-committed` through one shared reader (absent or illegal declarations resolve to the full chain -- every pre-168 session grandfathered by construction). `change_class` is now persisted in plan.schema.json (closing the prose-only gap). Self-application: Phase 168 touches L3 paths, so its own guard classified it full-chain -- the ceremony it traversed. chain.md tier section + delegation-table row added.
