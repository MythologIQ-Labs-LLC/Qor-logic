...
**Phase**: Phase 247 (hotfix; negative-constraints remediation; v0.163.1). Closes the Phase 240 independent reviewer's post-merge findings that a messaging race allowed onto main: the fabrication-protection doctrine's applicability no longer keys on the retired `min_model_capability` field (an unsatisfiable predicate) -- NR-001/NR-002 bind unconditionally for the fabrication-risk skill set independent of model identity, host, tier, or provider; the compiled preamble is corrected across all six host variants; the retired-field guard flags either field alone; the synthetic fixtures drive the shared detectors (red-proved); the execution-context audit seam is tested unstubbed. Entry #642's over-claimed report assertions are corrected in the record without rewriting sealed entries.
