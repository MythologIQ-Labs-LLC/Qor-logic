...
**Phase**: Phase 288 (hotfix; a stale session marker is not the same as an absent one; v0.174.1; GH #483). `session.py`'s `_marker_fresh` collapsed a marker that does not exist with one that exists, carries a valid session id, and is merely older than `SESSION_TTL` (24h) into a single boolean; both `current()` and `get_or_create()` treated the two identically, so a session whose gate directory still held unsealed, in-flight phase artifacts could be silently rotated out from under them the moment its marker's mtime alone aged past a day. `_marker_state` now distinguishes "absent"/"stale"/"fresh", and a new `_has_unsealed_gate_artifacts` check lets a stale-but-still-live session recover (reused, mtime refreshed) instead of rotating; a stale marker whose session is sealed or has no gate directory still rotates exactly as before. Independent Option B review (architect-reviewer subagent, author-momentum risk auto-dispatched by `audit_risk_score`) found no defect.

**Prior phase**: Phase 287 (feature; a verifier reports what it examined, and examining nothing is not a pass; v0.174.0). Three gates reported success over a scope that was empty, partial, or supplied by the party they audit, and nothing in the result said so: `plan_grep_lint` examined zero citations on every plan written to the house convention because `_LD_HEADING_RE` missed the `### LD-<n>` subheadings plans actually use; `ledger_commitment.stale_commitments([])` returned `[]`, indistinguishable from a thorough pass, while 105 of 204 sealed phases named no plan or brief at all; the install-sync gate compared 32 SKILL.md files and not 26 under `references/`, silently. Phase 219's scope-disclosure property (established for the publication-boundary lint) was generalised to all three.
