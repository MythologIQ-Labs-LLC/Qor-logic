...
**Snapshot**: 2026-06-10
**Chain Status**: ACTIVE. Phase 158 (feature, v0.110.0) closed the LAST Sprint A item -- non-forgeable gate-artifact provenance (GAP-GOV-05) -- and the GH #210 umbrella; Phase 159 (hotfix, v0.110.1) fixed GH #223 so a downstream `plan-<slug>.md` filename no longer hard-blocks the seal-entry consistency check; this entry (Phase 160, hotfix, v0.110.2) is a documentation-currency pass for the 158/159 work plus an enforcement test. (PyPI backfilled to a continuous 0.109.0->0.109.5 + 0.110.0->0.110.2 line on 2026-06-10.)
**Phase**: Phase 160 (hotfix; documentation currency). The README doctrine inventory had drifted to 34 of 35 `doctrine-*.md` files (omitting `provenance-binding`); `badge_currency` did not catch it because it checks the badge COUNT, not the prose table. This phase adds the missing README row, documents the Phase 158 `provenance-attest` CI job + per-session provenance sidecars and the Phase 159 seal-entry plan-name fallback in `docs/operations.md`, and adds `tests/test_readme_doctrine_inventory.py` pinning the README doctrine table to the on-disk corpus bidirectionally so it cannot silently drift again. No runtime behavior change.
