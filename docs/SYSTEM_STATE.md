...
**Snapshot**: 2026-06-10
**Chain Status**: ACTIVE. Phase 159 (hotfix, v0.110.1) fixed GH #223 (seal-entry plan-name fallback); Phase 160 (hotfix, v0.110.2) was a documentation-currency pass for the 158/159 work plus an inventory-enforcement test; this entry (Phase 161, hotfix, v0.110.3) makes `tests/test_merge_velocity_check.py` deterministic, closing a `PYTHONHASHSEED`-coupled CI flake. (PyPI is a continuous 0.109.0->0.109.5 + 0.110.0->0.110.x line as of 2026-06-10.)
**Phase**: Phase 161 (hotfix; CI flake). `tests/test_merge_velocity_check.py::_make_merge_commit` derived its branch + filename from `abs(hash(subject)) % 100000`; Python's `hash(str)` is `PYTHONHASHSEED`-randomized and the mod-100000 truncation collides distinct subjects, so the second `git checkout -b feat-<n>` could hit an existing branch and exit 128 -- an intermittent failure that flaked the ubuntu-3.13 matrix cell during the Phase 158 PR (worked around twice by re-running). Replaced both sites with a pure `_feat_suffix(subject) = hashlib.sha1(subject).hexdigest()[:8]`: deterministic across processes, collision-resistant. Verified green under `PYTHONHASHSEED=0` and `=12345`. Test-only; no runtime behavior change; closes a test-discipline-doctrine (no-hidden-random-coupling) violation.
