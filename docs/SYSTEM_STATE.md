...
**Snapshot**: 2026-07-04
**Chain Status**: ACTIVE. Phase 163 (hotfix, v0.111.1) gated the PyPI publish on CI success for the tagged commit; Phase 164 (feature, v0.112.0) made seal presentation artifacts generated-not-asserted (merged + tagged 2026-07-04); this entry (Phase 165, feature, v0.113.0) adds the autonomous QA nightly: a machine-readable aggregate health verdict plus a scheduled workflow with automatic GitHub-issue lifecycle. (PyPI is a continuous 0.109.0->0.111.0 line, + 0.111.1/0.112.0 in flight pending `pypi` approvals, as of 2026-07-04.)
**Phase**: Phase 165 (feature; autonomous QA nightly; research entry #382, GH #250 part a + GH #240). Governance health now verifies itself with zero operator engagement. New `qor/scripts/status_json.py` (145 lines, stdlib) runs the read-only ladder (governance-health, ledger chain, seal-artifact currency, gate-chain completeness, provenance verify-committed, governance index) in-process with a signature-aware module runner, prints human lines then exactly ONE JSON object as the final stdout line (`{schema_version, ts, checks:[{id, ok, exit, summary}], overall_ok}`; exit 0 iff all ok) -- closing GH #240 -- and carries a `--self-test` mode that validates the aggregation logic against a synthetic pass+fail registry before any consumer trusts a live verdict. New `.github/workflows/nightly-health.yml` (cron 09:00 UTC + workflow_dispatch; permissions contents:read + issues:write only) runs self-test -> aggregate -> packaging smoke, mirrors output to the step summary, and keeps exactly one "Nightly governance health" issue in sync: drift comments-or-creates (labels bug+governance, body carries the JSON payload via env-var passing per the audit's OWASP A03 binding note), recovery closes with a resolved comment. GH #250 stays open for the dry-run follow-on (half-measure-closure countermeasure).
