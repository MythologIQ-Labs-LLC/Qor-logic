...
The entry condition was measured before the flip (three WARN, zero EXCEEDED, exit 0), so the wall costs nothing today and structurally closes the drift class that consumed hard-constraint incidents at Phases 222 and 229. The new posture is bound through the substantiate_gates parser red-then-green, the exact-literal guardrail flipped with the row per the Phase 129 precedent, and the superseded V1 command survives verbatim in the seal-gate-ladder reference as V2 history. This seal was the flipped gate's first live run.

The `latest <= got <= latest + 1` window in `_check_header` is retired. Tightening it to equality showed the window had been concealing the same pre-append write on the header dimension rather than tolerating it. The superseded wiring test pinned the invocations to Steps 6 and 6.5 -- it had pinned the defective order, which is why eight phases of green did not mean the sequence was right; the replacement asserts both bounds.

On this seal the relocated gate reported `README declares 578, truth 582` before regenerating. Under the old order that state was unreachable by the check.
