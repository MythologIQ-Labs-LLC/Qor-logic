...
**Phase**: Phase 298 (hotfix; dependency review covers every governed dependency file; v0.175.1; GH #511). The PR Dependency Review workflow now triggers on `requirements-release.in`, `requirements-sbom.in` and `requirements-sbom.txt` in addition to `pyproject.toml` and `requirements-release.txt`, and runs one hard-fail cooling-period admission step per governed root lockfile (`--lockfile requirements-release.txt`, `--lockfile requirements-sbom.txt`). The workflow regression derives the governed set from the repository root; new CLI tests prove `dependency_admission_lint.main` examines the named lockfile. No `qor/` code changed; pyproject-only pins remain unexamined in CI (declared residual).
