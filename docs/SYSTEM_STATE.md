...
**Snapshot**: 2026-07-04
**Chain Status**: ACTIVE. Phase 165 (feature, v0.113.0) added the autonomous QA nightly; Phase 166 (feature, v0.114.0) made SG closure require an executable enforcer; this entry (Phase 167, feature, v0.115.0) completes the autonomous-QA cluster: every mutating CLI surface now has a dry-run rehearsal mode (GH #250 closes at full scope). (PyPI is a continuous 0.109.0->0.111.0 line, + 0.111.1/0.112.0/0.113.0/0.114.0 in flight pending `pypi` approvals, as of 2026-07-04.)
**Phase**: Phase 167 (feature; dry-run modes; research entry #390, closes GH #250). Keyword-defaulted `dry_run` guards at every write funnel: `seal_artifacts --write --dry-run` (previews the write set), `reconcile propose/authorize --dry-run` (runs the full drift guard + computes the RECONCILIATION entry preview without touching ledger or proposal), changelog `apply_stamp`/`stamp` `dry_run=` (validation raises identically -- an empty Unreleased still fails in dry mode), `governance-index --advance-last-reviewed --dry-run`, `uninstall --dry-run` (lists the removal set). Reads, rendering, and validation execute exactly as wet; each suppressed mutation prints one `[dry] would <action>` line (the Orko rehearsal pattern). New operator-facing `qor/scripts/session_tool.py` (`current` / `rotate --dry-run`) gives safe session inspection while the internal automation `rotate()` stays mutation-only. `qor/cli.py` grew exactly one flag line (uninstall, mirroring install). GH #250 closes at full scope: parts a/c shipped in Phase 165 with live runtime evidence, part b here.
