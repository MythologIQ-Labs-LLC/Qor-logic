...
**Phase**: Phase 286 (feature; the filing path reports what it is about to publish; v0.173.0). `create_shadow_issue` composed a GitHub issue body from shadow events and handed it to `gh` with nothing inspecting it. `qor/scripts/advisory_filing_control.py` now scans the composed title and body, prints what it found, records a gitignored calibration line, and files regardless.

Advisory by construction rather than by intention. `inspect` never raises: the whole body is wrapped and any internal failure yields an empty report carrying a `control-error` class, because a control that observes must not become the reason a report cannot be filed. Identity anchors on the directory the events were read from, so the repository whose events composed the body and the one whose identity is matched against it cannot diverge. The record is a JSONL line rather than a shadow event -- `gate_override` is the only event type carrying a raising override-friction escalator, so recording there would have let this control refuse a filing at the third occurrence and corrupted the population `override_friction.check` reads.

The blocking design that preceded it was vetoed three times across fifteen authoring iterations and five independent reviews: first for documentation drift, then for design defects the drift had hidden, then for defects in the fixes. Research then measured the path and found the justification belonged to a different channel -- the seven-of-seven leak rate is the failure rate of hand-filed reports, while this path has filed once, cleanly, and an advisory scan of the live 591-line body reports zero findings. Blocking is deferred until this phase supplies a refusal rate to calibrate against.

Four tests that could not fail shipped across that history, every one written while closing a different finding. The Test Functionality pass is now mechanical: each descriptor must state the implementation that would make it fail, checked by pattern. One design defect was caught here by a red test rather than by review -- `origin_pair` swallowed git-unavailable as though it were no-remote-configured, so a scan that had lost two of its three derived terms looked identical to a clean one.
