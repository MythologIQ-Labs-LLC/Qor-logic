...
**Phase**: Phase 275 (feature; one definition of the audit verdict; v0.171.0). Two gates read `.agent/staging/AUDIT_REPORT.md` with different regexes and disagreed about its verdict on 84 of the 189 reports the intent lock accepted. `intent_lock` accepted a PASS-shaped line anywhere in the file regardless of the verdict asserted, so a report quoting the canonical form while carrying a VETO authorized capture (GH #424); `verdict_reconcile` could read neither the heading form nor a bolded value nor a backticked or qualified target, so its wired ABORT refused valid work (GH #462). Both now read one shared dialect, agreement-required on both fields, with a label probe strictly wider than the value reader so an unparseable field is reported rather than dropped -- a dropped line cannot conflict, which is how the original defect worked.
