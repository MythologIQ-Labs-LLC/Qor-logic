...
**Phase**: Phase 242 (feature; portable governed-procedure execution evidence; v0.160.0; GH #384). Canonical Qor owns the meaning and satisfaction rules of procedure-execution evidence: `qor/compliance/procedure_evidence.py` evaluates policy-derived requirement sets deterministically and fail-closed -- evidence binds to exact procedure/subject/input digests; independent classes (`wrapper-observed`, `ci-attested`) require declared trusted principals plus externally verified whole-claim sha256 digests, so an agent-fabricated receipt cannot satisfy them; `indeterminate`/missing never reads as satisfied. Published schema `procedure_execution_evidence` (registered) now encodes the independent-class trustedPrincipals rule so schema-only downstream validators carry the same meaning; the semantics reference declares the requirements-authorship trust-domain precondition (policy must originate from a domain the evidence producer does not control). FEATURE_INDEX FX026; 18 behavioral tests, 15 of them rejection/error paths.

The revalidation tribunal's independent reviewer confirmed the fabrication threat model holds (VerifiedClaim is a function argument, never a JSON field; additionalProperties closed) and both substantive findings -- the schema/reference drift and the unstated policy-authorship trust boundary -- were closed in this promotion rather than deferred.
