...
**Snapshot**: 2026-07-04
**Chain Status**: ACTIVE. Phase 162 (feature, v0.111.0) shipped the WARN-first ledger base-currency gate; Phase 163 (hotfix, v0.111.1) gated the PyPI publish on the CI suite passing for the tagged commit (merged to main + tagged 2026-07-04; publish pending `pypi` environment approval); this entry (Phase 164, feature, v0.112.0) makes seal presentation artifacts generated, not asserted. (PyPI is a continuous 0.109.0->0.111.0 line, + 0.111.1 in flight, as of 2026-07-04.)
**Phase**: Phase 164 (feature; generate-not-assert; research entry #378 rec 2). The seal ceremony hand-edited README count badges and the SYSTEM_STATE header while 13 always-on tests asserted that live repo state matched moving truth -- the class that broke on nearly every seal (phases 121/122/123/140). New `qor/scripts/seal_artifacts.py` (pure renderers + atomic updaters reusing the `badge_currency` counters; 179 lines, stdlib only) regenerates the five README literal-count badges and the SYSTEM_STATE Snapshot/Phase fields deterministically (`--write`) and verifies currency fail-closed (`--check`). Substantiate Steps 6/6.5 now invoke it (ABORT + hotfix exemption retained); a new CI step (`seal-artifacts currency`, gate-chain-completeness job) enforces the same check on sealed state. The 13-test live-equality class (5 badge-equality + 2 header-currency + 6 prose-wiring) is retired for 9 behavioral generator tests against synthetic fixtures + 1 exempted wiring lock; compliance control `badge-currency` re-pointed to the new module; Phase 89 CI-surface registry extended (forward-maintenance). Currency enforcement now lives where repo state is stable -- at seal time and in CI -- instead of breaking mid-phase.
