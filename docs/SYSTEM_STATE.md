...
**Phase**: Phase 234 (feature; size-budget V2 flip; v0.155.0; closes GH #320). The Step 4.6.9 skill-corpus size-budget gate converted from WARN to hard ABORT: the ladder row dropped its true-suffix escape, so a governance skill crossing the 40 KB EXCEEDED threshold can no longer seal. WARN-band findings (25 KB) remain advisory; no override path exists because a size breach is self-inflicted, double-signaled ahead of the wall, and resolves via the progressive-disclosure refactor recommended since Phase 95.
