...
**Phase**: Phase 510 (hotfix; reconcile.py reads the dialect accessor instead of indexing its groups; v0.174.2; GH #477). `reconcile.py`'s `detect_residual` and `_recorded_chain_hash` indexed `ledger_dialect`'s owned `_HASH_VALUE` capture-group layout directly (`match.group(1) or match.group(2)`), silently dropping the third recognized hash-value form -- a bare 64-hex digest alone on its own line. Both now call the published `ledger_dialect.hash_value` accessor, matching `ledger_hash.py`'s existing behavior at its own equivalent call sites. Recomposes PR #493's already-independently-reviewed fix onto current `main` after Phase 290 landed the `0.174.1` slot #493 had independently claimed against a stale base.
