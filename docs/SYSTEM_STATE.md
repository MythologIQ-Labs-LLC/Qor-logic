...
**Phase**: Phase 243 (feature; governed promotion of completed relay hotfixes; v0.158.0; GH #389). Consolidates eight completed relay fixes onto one governed head: ledger-fork enforcement with duplicate-entry-number FAIL on both verify and post-anchor surfaces plus gated sequence-break tolerance (GH #361); labeled-but-unparseable hash fields fail post-anchor verification (GH #363); gate_chain_completeness zero-population honesty (GH #366; phase-optional-selection residual split to GH #395); FEATURE_INDEX `Status` alias (GH #365); consumer-extensible secret-scanner allowlist with stderr disclosure, default posture unchanged here (GH #359); `repeated_veto_pattern` classifier rule and remediation proposal (GH #362); closure_enforcer behavioral validation with a disclosed shape-only fallback for corpus-less consumer trees (GH #364; semantic-relevance residual split to GH #396); qor-audit skill-size headroom via progressive disclosure plus the dist recompile the relay branch omitted (GH #357).

The tribunal ran two iterations. Iteration 1 (VETO, independent reviewer): the GH #364 batch-blocking test passed for the wrong reason (its intended-valid gate-step member failed corpus resolution under the test's own repo_root, so the four-forms rejection was never exercised), and the tightened gate-step form globbed a source-tree-only path that would have made every consumer workspace's gate-step enforcer raise. Iteration 2 closed both: the test stages a real Step heading and was red-proved by neutering the rejection; the validator falls back to disclosed shape-only acceptance where no skill corpus exists, with a consumer-shaped-tree regression test.
