...
**Snapshot**: 2026-06-09
**Chain Status**: ACTIVE. Phase 144 (feature, v0.107.0) authored the FEATURE_INDEX + a README currency fix; Phase 145 (hotfix, v0.107.1) wired the #201 UTF-8/ASCII validity gate into the seal path; this entry (Phase 146, hotfix) backfills behavioral test coverage for the `list`/`info` CLI commands and seals at v0.107.2.
**Phase**: Phase 146 (hotfix, FEATURE_INDEX backfill). Added behavioral tests for the `qor-logic list` and `qor-logic info` handlers (`tests/test_cli_feature_index_backfill.py`) and corrected the `qor-logic uninstall` FEATURE_INDEX row to cite its existing verifying test, taking `docs/FEATURE_INDEX.md` to all 17 features `verified` (was 14 verified / 3 unverified). Preceded by a research brief (stash triage + backfill scope) that confirmed all 10 residual git stashes are droppable dist/install/transient cruft. Full per-phase history is authoritative in `docs/META_LEDGER.md` (352 entries; latest Entry #352 -- Phase 146 -- v0.107.2 -- chain `<sealed below>`); SYSTEM_STATE keeps the current-state header, physical map, and a condensed recent-phase bridge rather than restating every phase. The pre-1.0 line is current: the half-measure-closures cluster (GH #147 + #148-#165) is fully closed with real enforcers, and the two largest governance skills remain under the 40 KB skill-size budget (Phases 135/136).
