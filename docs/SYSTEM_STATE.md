...
**Phase**: Phase 291 (hotfix; reconcile.py reads the dialect accessor instead of indexing its groups; v0.174.1; GH #477). `reconcile.py`'s `detect_residual` and `_recorded_chain_hash` indexed `ledger_dialect`'s owned `_HASH_VALUE` capture-group layout directly (`match.group(1) or match.group(2)`), silently dropping the third recognized hash-value form -- a bare 64-hex digest alone on its own line. Both now call the published `ledger_dialect.hash_value` accessor, matching `ledger_hash.py`'s existing behavior at its own equivalent call sites: the last consumer bypassing the accessor now reads through it. Latent (no live ledger entry used the bare-line form), closed with two red-then-green regression tests.

**Prior phase**: Phase 287 (feature; a verifier reports what it examined, and examining nothing is not a pass; v0.174.0). Three gates reported success over a scope that was empty, partial, or supplied by the party they audit, and nothing in the result said so.
