...
**Snapshot**: 2026-07-04
**Chain Status**: ACTIVE. Phase 167 (feature, v0.115.0) completed the autonomous-QA cluster; Phase 168 (feature, v0.116.0) made gate depth scale with declared risk; this entry (Phase 169, feature, v0.117.0) inverts the evidence posture: audit evidence is reconstructed on demand from recorded signals, and net-new ceremony artifacts are frozen behind a registry + justification rule. (PyPI is a continuous 0.109.0->0.111.0 line, + 0.111.1 through 0.116.0 in flight pending `pypi` approvals, as of 2026-07-04.)
**Phase**: Phase 169 (feature; evidence reconstruction; research entry #398, closes GH #251 -- the Decision-BOM posture from MS agent-governance-toolkit ADR 0018). New `qor/scripts/evidence_bundle.py` (234 lines): `--session <sid>` or `--phase <N>` resolves via the ledger's SESSION SEAL blocks and joins the eight already-recorded signals (ledger entry; gate artifacts honoring Phase 168 short-chain declarations via `tier_guard.declared_artifacts`; provenance sidecars; audit_history.jsonl; intent-lock record; shadow events across BOTH local and upstream logs; CHANGELOG section; seal commit/tag) into one final-line JSON bundle. Collectors never raise; `completeness.missing` NAMES absent signals -- partial reconstruction is surfaced, never fabricated. Companion freeze rule: `qor/gates/SCHEMA_REGISTRY.json` baselines the 19 existing gate schemas; WARN-only `gate_schema_freeze_lint` (audit Step 0.6 ladder) flags net-new schemas lacking registration or a plan-declared `new_ceremony_artifacts` justification (>= 100 chars; new optional plan.schema.json field). Live self-application: Phase 168 reconstructs 8/8 signals; the freeze lint reports 0 unjustified schemas. Zero new ceremony artifacts introduced by the phase itself.
