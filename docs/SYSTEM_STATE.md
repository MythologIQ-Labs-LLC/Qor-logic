...
**Phase**: Phase 240 (governance; execution-context adaptive governance; version-not-applicable; GH #379). Replaces the Phase 55 model-name pinning seam with an execution-context contract (`qor/scripts/execution_context.py`): model identity is provenance rather than execution authority; skill frontmatter retires `model_compatibility`/`min_model_capability` for `advisory_quality_requirements`/`rendering_recipes`/`default_rendering_recipe`; hard capability absence binds only when capability telemetry is explicitly complete; rendering adaptation is bounded and presentation-only; fabrication protections remain independent. Behavioral spec at `qor/specs/execution-context-governance/spec.md` with a session delta.

The branch's bounded tribunal harness ran locally against product head 3dd51126 after its CI form tripped on in-run dist regeneration; it recorded PASS at entry #642 and retired itself. Rebase integration onto v0.158.0 main required dist-manifest reconciliation and recovery of qor-audit skill-size slack under Phase 243's new usable-slack guard (description tightened; closed-enum coverage detail relocated to phase37-subpasses.md with the binding VETO rule kept inline and both glossary term references registered).
