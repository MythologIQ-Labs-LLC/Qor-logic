...
**Phase**: Phase 279 (hotfix; the marker's writer owns its removal; v0.171.3). `create_shadow_issue --flip-only` deleted the breach marker unconditionally, discarding the `flipped` count it had just printed, so a well-formed id matching no event destroyed the breach record and returned 0. The marker asserts that the unaddressed severity sum exceeds the threshold; this command never reads it, and its ids come from a cross-repo sweep that never consulted this repository's sum. Removal returns to `check_shadow_threshold`, its sole writer. No condition was substituted: every one available is a proxy for a comparison the command lacks the inputs to make, and both candidates were refuted with constructed counterexamples.

Three VETOs on a two-line deletion, none of them about the code. Four claims were asserted without measurement, three of them inside a remedy for the previous one -- recorded as a `repeated_veto_pattern` whose rule is not "check your numbers" but stop restating numbers an artifact owns. A second entry records something new: a test can be correct when written and hollowed out by a later phase that never touches it, which is what this change did to Phase 278's placement test before it was re-pinned on control flow.

Four review rounds. The plan asserted two entry points where there are four; measured its cost over event ids and applied it to split elements, where a trailing comma and surrounding whitespace both fail; and wrote a test row asserting only an exit code, which passes with the guard placed after the destructive unlink it is meant to precede. The normalizer is three rules rather than two because conflating them reintroduces the defect through the fix. A neighbouring destructive defect -- `--flip-only` deleting the breach marker whether or not it flipped anything -- is filed at GH #472 and explicitly not claimed.

The filed remedy for GH #467 would have broken a seal gate: `ledger_commitment` is deliberately narrow because its citation pattern accepts Plan and Brief, so widening admits 45 session-seal digests as commitments to plans they do not describe -- 0 of 45 match the cited file, against 99 of 147 for the plain form. That module keeps its pattern and gains the comment its invariant never had. Four review rounds also caught a substitution that would have left one field untouched while appearing fixed, four numbers wrong in the flattering direction, and a planned test that would itself have aborted the seal.

Six review rounds produced nineteen findings and eight recorded failure modes, seven of them the same mistake: the fix scoped to the example in hand rather than the class. One arrived inside the fix for the one before it, and one measurement script contained the very defect it was measuring. Four adjacent defects were filed rather than folded in, the sharpest being that a wired ABORT gate produced no observable effect across a continuous run of sealed phases (GH #463), so nothing distinguishes a gate that passed from one that never ran.

The Shadow Genome gains the parallel-reviewer-racing pattern: never take the action a pending reviewer could forbid while any dispatched reviewer's verdict is outstanding; a replacement dispatch does not supersede the original. Closure of the raced-in findings executed the originating reviewer's own cited checks after their documented non-response.

**Prior phase**: Phase 246 (feature; /qor-implement changeset simplification profile; v0.163.0; GH #392 final tranche). Step 9.5 profile + Step 9 rerouted through the Simplification Test gate; closed GH #392 and the eight-phase promotion arc.

**Prior phase**: Phase 245 (feature; /qor-refactor modernization; v0.162.0; GH #392 Tranche A). Behavior-preserving simplification contract with the corrected completion gate and Simplification-Test-routed thresholds.

**Prior phase**: Phase 239 (feature; governed promotion of the /qor-roadmap P1 pilot; v0.161.0; GH #373). The pilot reached main through its own two-iteration tribunal; PR #378 superseded; evaluation remains GH #373's next task.

**Prior phase**: Phase 242 (feature; portable governed-procedure execution evidence; v0.160.0; GH #384). Fail-closed evaluator + published schema carrying one canonical meaning; fabrication threat model verified; requirements-authorship trust precondition declared.

**Prior phase**: Phase 241 (feature; portable governance engine boundary; v0.159.0; GH #381). Base owns portable governance semantics; downstream strengthens but never redefines evidence meaning; stale carried audit artifacts refused at revalidation; boundary tests hardened to contract-level assertions.

**Prior phase**: Phase 240 (governance; execution-context adaptive governance; version-not-applicable; GH #379). Model identity became provenance rather than execution authority; the bounded tribunal ran locally, recorded PASS at entry #642, and retired itself; the post-seal independent-reviewer amendment (#645) delivered the live-corpus retired-field test and per-skill contract-error accumulation.

**Prior phase**: Phase 243 (feature; governed promotion of completed relay hotfixes; v0.158.0; GH #389). Eight relay fixes consolidated and promoted through a two-iteration tribunal (independent-reviewer VETO on two GH #364 defects, closed with an executed red-proof and a consumer-tree fallback); residuals split to GH #395/#396.

**Prior phase**: Phase 244 (feature; /qor-harden implementation-quality capability; v0.157.0; GH #388). Adds `/qor-harden` as a standalone, environment-neutral implementation-quality review/repair skill with nine canonical IQ dimensions, the reusable sweep protocol at `qor/references/implementation-quality-sweep.md`, unified `qor/references/doctrine-code-quality.md`, and full registry/help/delegation/README/dist wiring. Its tribunal's independent-reviewer iteration caught IQ-CONTEXT missing from both selection surfaces and a vacuous glossary gate reached through the non-canonical `terms_introduced` artifact key (framework defect filed as GH #394); both closed with a derived-set binding test and a negative-probe-verified live gate.

**Prior phase**: Phase 235 (feature; canonical consumer-contract fixtures; v0.156.0; GH #358). Published `qor/fixtures/consumer-contract/` -- canonical fixtures proving the six external-consumer states (supported, missing-optional, stale, malformed, unsupported-version, partial-migration) for the artifact types this repository actually owns and produces (META_LEDGER, FEATURE_INDEX, gate artifacts), each proven tests-first against this repository's own real parsers/schemas. Two scope corrections were disclosed rather than silently reconciled: the originating ask's fourth artifact type, "tracker manifest / programs.yaml," has no Qor-logic schema, parser, template, or producer anywhere in this repository and is recorded `out_of_scope`; two further per-state cells (FEATURE_INDEX stale/unsupported-version, gate-artifact unsupported-version) are recorded as disclosed gaps for the same reason.
...
The fix refused its own author. This seal hit NO LOCK for the fifth consecutive phase, and the friction shipped in the same phase rejected the unjustified record, naming the recurrence and demanding a justification. Dogfooding also caught an off-by-one: the threshold was chosen to fire on the third occurrence, and comparing prior-count alone charged the fourth -- exactly when the operator already knew.

`qor-substantiate` is at the end of its rope. A single comment line took it three bytes over the lock; the wiring was not shaved and three bytes came from prose instead, landing 39,912 with 24 of slack. Three consecutive phases have each needed a relocation round to fit one step. The next phase to touch it should expect a structural remedy.
**Prior phase**: Phase 219 (feature; unseen-surface gates; v0.142.0; closes GH #309, #311, #312). The filed defect was wrong twice. GH #309 said the boundary lint misses files STAGED but not committed and proposed a staged-only mode; `git ls-files` reads the index, so staged files were always scanned and UNTRACKED ones were not -- and a staged-only mode would have narrowed the surface to a subset of existing coverage, catching none of the four documented misses.

The real gap was ordering. `/qor-audit` ran the lint WARN-only over a tree predating implementation, CI ran it fail-closed but structural-only, and the seal ran nothing. No fail-closed, identity-aware run ever saw a phase's new files before they were committed. New Step 4.6.14 runs it fail-closed after staging; this seal is its first execution. The lint now reports its detector scope and the seal records it, because CI cannot load the gitignored identity overlay and an unqualified zero means less there -- disclosure rather than coverage, since coverage is impossible.

The disclosure pass on `qor-substantiate` took three rounds (28 bytes of slack to 176, to 392, a 133-byte breach, then 75). LD-3 bound the implementer to extend the pass rather than compress the step, which is why the deliverable survived intact.

Two process defects are now counted rather than excused: `intent_lock` overridden in four consecutive phases and `plan.json` backfilled at seal in two, both from driving the canonical skills manually and skipping their artifact-writing steps. The gates catch it every time and are overridden every time, which is what a control being trained around looks like. Recorded at severity 2 and filed.
**Prior phase**: Phase 218 (feature; unreconciled-record cluster; v0.141.0; closes GH #313, #316, #318, #321 under umbrella #319). Four governance checks were returning success on input they exist to catch. A failing check is a working control; a passing check on bad input certifies the defect.

`ledger_hash verify` accepted a ledger with an entry DELETED -- tampering was already caught, but every survivor of a deletion stays internally consistent, so per-entry arithmetic saw nothing. A cross-entry sequence assertion over file order closes it. `intent_lock` hashed raw bytes while `ledger_hash` had normalized line endings since GAP-GOV-03. `/qor-implement` Step 2 branched on a verdict string with no comparison to its gate artifact, and because the staging directory is not session-scoped a stale report survived into three separate phases this session. `gate_provenance` reported success over a sidecar whose digest did not recompute, because `-iterN` artifacts sat outside a walk that reused a completeness list as a verification scope.

Each fix ships a counterfactual test confirmed failing beforehand. All four checks previously passed on the input they should reject, so a good-path test would have proven nothing.

The audit VETOed on a factual error the plan asserted rather than measured: a SECOND ledger gap at #510, which surfaced only when the file-order question was tested against live data. The remedy derives the gap set from the ledger and asserts equality with the declared constant, so widening it to silence a new gap goes red.

Three defects were caught in-cycle. The sequence assertion collided with the grandfathering mechanism and would have shipped as a fail-closed regression against the project's own documented residuals. Dogfooding `verdict_reconcile` against this phase's own artifact exposed a path-separator false positive that would have blocked every implement on Windows. And two recording failures -- an uncaptured intent lock, second consecutive occurrence, and a `plan.json` backfilled at seal time -- are disclosed as gate_override events rather than papered over.
**Prior phase**: Phase 217 (feature; installed skill-corpus drift; v0.140.0; rescopes GH #314). 27 of 30 skills differed between this repository and the operator installed copies -- the copies that actually execute. The tool that detects exactly this has existed since Phase 32, works correctly, and did not fire: wired at `--scope repo` while the install was global, wrapped in `|| echo`, and run only at plan time.

The defect was noise rather than silence. Pointing at a non-existent directory produced 30 findings per run, one per source skill, all expected and none meaningful, hiding 27 real mismatches behind them. A control whose default output is guaranteed-irrelevant trains the operator past it, so noise and silence fail identically. `install_drift_check` gains `installed_scopes()` and `scope="auto"`; an absent scope reports once. New `skill_corpus.digest()` lets Step 4.6.13 record which ceremony produced each seal -- across 544 prior entries nothing answered that.

Disclosure, not enforcement: the skill running the check is inside the corpus under test, so the drift most worth catching is the drift that removes the catcher. Enforcement is tracked at GH #320, filed before re-audit rather than deferred to an unfiled V2.

Three self-inflicted defects were caught in-cycle. Step 4.6.13 was wired at `--scope auto` while `auto` existed only in the function and not in the CLI argparse choices, so the remedy for an inert control was itself an inert control -- caught because this ceremony ran the step instead of assuming it. The absent-scope collapse first broke a shipped contract for partial installs. And the new tests read the operator real home directory, which is the same dishonesty this phase corrects. The intent lock was never captured and was NOT back-dated; the substantive claim was verified independently against audit entry #543 and the miss recorded as a gate_override.
**Prior phase**: Phase 216 (feature; provider-neutral execution-continuity semantics; v0.139.0; closes GH #285). The lifecycle can now plan, audit, implement, substantiate, validate, and remediate continuation of one governed claim across a change of execution provider. A provider stopping for budget, context, capacity, credentials, or environment is resumable by an authorized successor when a valid checkpoint exists -- not product failure, and not human escalation by default. New `continuity_gate.py` classifies evidence into verified/rejected/inconclusive plus a routing directive; `continuity_contract.py` holds the Qor-owned key set and the config-backed pin; `plan_continuity_lint.py` validates the declaration. Schema: `execution_continuity` on plans with `additionalProperties: false`, `continuity_outcome` on validate and remediate.

Two distinctions carry the design. `inconclusive` is not `skip`: skip means a gate deliberately did not run and is acceptable to seal, while inconclusive means the gate ran and the evidence environment denied it a conclusion, routing to environment repair. And revision comparison is exact equality, never git ancestry -- `intent_lock` accepts an ancestor by design since Phase 43, and a receipt inheriting that tolerance would accept precisely the stale evidence #285 forbids. One test asserts an ancestor-revision receipt is REJECTED; it is the proof the two mechanisms stayed separate. The upstream contract is referenced by version and never duplicated, and the pin is asserted rather than verified, because conformance checking would require holding the schema the ownership boundary forbids copying.

Phase A (Phase 215) was sized correctly and only just: wiring the seven skills consumed 1084 bytes of qor-audit and 807 of qor-substantiate against a research estimate of 520 and 360. Recovering only that minimum would have breached the 39936-byte lock here.

Three CRLF-class defects surfaced. `Path.write_text` translates newlines on Windows, so 65 edited files acquired CRLF and the headroom test read both governance skills as over-lock; normalizing to LF then made `intent_lock` ABORT with audit drift. Re-CRLF-ing the current report reproduced the stored hash exactly, proving the change was encoding-only, and the re-capture is recorded as a gate_override. The root cause is that `intent_lock` hashes raw bytes while `ledger_hash.content_hash` normalizes to LF (ledger_hash.py:37) -- the GAP-GOV-03 lesson reached one hasher and not the other.
**Prior phase**: Phase 216 (governance; governance-skill headroom recovery; Phase A of GH #285). `qor-audit` and `qor-substantiate` sat 1016 and 1176 bytes above a 38400-byte working target beneath the 39936-byte test-enforced lock, leaving no room for the execution-continuity semantics Phase B adds. Rationale was relocated into already-cited references as titled subsections with inline pointers left behind: `qor-audit` 39416 -> 38389 (-1027), `qor-substantiate` 39576 -> 38816 (-760). No test was edited, weakened, or skipped -- the guardrail suite IS the specification being preserved, so a relocation pass that adjusts a failing guardrail proves nothing.

The phase's finding outweighs its byte count. `qor-substantiate` cannot reach 38400 without moving operative prose: of 2786 bytes of unpinned prose, roughly 1567 carries ABORT clauses, escape idioms, or operator actions, leaving ~1219 genuinely explanatory against 1176 needed -- a 43-byte margin over a heuristic classification. The plan was amended mid-implementation (entry #535) to set that file a 38876-byte target with real margin rather than let a numeric goal pressure the reclassification of a seal step as rationale. A future pass on that file needs a structural remedy, not another relocation round.

Two defects surfaced during the ceremony. `.agent/staging/AUDIT_REPORT.md` still carried the Phase 206 report while the Phase 215 gate artifact recorded PASS -- `/qor-implement` Step 2 reads the report and would have found a PASS for the wrong phase; nothing cross-checks that the two records name the same target. And Step 4.6.11 of the seal ceremony mandates `|| ABORT` on `qor.scripts.instruction_hygiene_lint`, a module absent from this repository entirely -- no source, no tests, no git history -- despite the step declaring itself Phase 208 wiring. Recorded as a disclosed SKIP per Phase 75 declarative tolerance.
**Prior phase**: Phase 215 (feature; declarable badge-count topology; v0.134.0). The README count badges resolved against a hardcoded `qor/` topology, and a missing directory produced `0` rather than an error, conflating "this repository declares an empty layout" with "this repository's layout could not be resolved" and letting a release-class phase seal against false zero counts (GH #293). New `qor/scripts/badge_layout.py` carries the topology as one frozen `BadgeLayout` whose field defaults are the existing `qor/skills`, `qor/agents`, `qor/references` roots, so an adopter that declares nothing sees no change; `badge_currency` and `seal_artifacts` consume that same value through one `layout` keyword or the shared `--skills-root` / `--skills-pattern` / `--agents-root` / `--agents-pattern` / `--doctrines-root` / `--doctrines-pattern` flags, so writer and checker cannot disagree about which topology they measured. An unresolvable root now fails loudly and names itself; a resolved-but-empty root still counts zero. Roots and matched files are confined to both the repository and the declared root, with traversing patterns, absolute patterns, and matched symlinks rejected. First iteration VETOed at entry #502 on Section 4: the layout had been introduced as six loose parameters repeated across seven signatures, which alone pushed `seal_artifacts.py` to 323 lines and three functions over 40. The value object and the module split were the audit-forced remedy (`seal_artifacts.py` 249, `badge_currency.py` 249, `badge_layout.py` 62; longest function 33). `update_files` now requires `counts`: its `counts=None` fallback was unreachable from the only caller and could not run without a `tests/` directory. This seal is the first regenerated by the repaired counters. Also cleared a pre-existing fail-closed gate inherited from main: `docs/ecosystem-position.md` was registered to Tier 5, having been added 2026-07-14 without an index row.
**Prior phase**: Phase 205 (feature; zero-cost OSS SAST + CodeQL security baseline). Entries #499-#501.
**Prior phase**: Phase 194 (feature; unified governance-path resolution + ledger dialect; v0.133.0). Entries #496-#498.
**Prior phase**: Phase 172 (feature; publication-boundary remediation; v0.119.0). Qor-logic's tracked and published surfaces no longer identify repositories outside this project. New `doctrine-publication-boundary.md` + structural lint `publication_boundary_lint` (absolute-path/non-self-URL/cross-repo-issue shapes; operator-local gitignored terms overlay at `.qor/private/boundary-terms.txt` so the term list is never tracked). Retroactive sweep: ledger entry bodies anonymized to neutral concepts (hash fields byte-untouched, chain verified; recorded hashes attest pre-remediation bytes retrievable in git history), 12 tracked gate artifacts swept with 7 provenance sidecars regenerated (`verify-committed --phase-min 158` green), ~300 docs/archive files, skill SOURCE.yml metadata, intent-lock records now repo-relative, and the GitHub surface (all open issues + every item created 2026-07-04; 79 historical items inventoried as GH #260). Exceptions preserved: two `license: Proprietary` skill lines + `qor/vendor/` third-party attribution (legally required text). The consumer-specific `qor-governance-compliance` skill left the public repo (registry now 29 skills / governance 5).
**Prior phase**: Phase 171 (hotfix; release evidence pins; v0.118.1). The release pipeline's `evidence.json` `action_pins` literals drifted from the pins actually used (checkout entry pre-dated even v6.0.3; both fell further behind after the dependabot v7.0.0/v6.3.0 bumps, PRs #235/#236, which also aligned nightly-health.yml via the rebased group). The two literals now equal release.yml's real `uses:` SHAs. FIRST LIVE SHORT-CHAIN PHASE under the Phase 168 tier rules: `required_gate_artifacts: [plan, implement, substantiate]` declared (L1-risk hotfix per tier_guard), audit legally skipped with severity-1 gate_override shadow event `b116f94a8f49`; the implement prior resolved to plan.json via the Phase 168 carve-out, and completeness/provenance validate the declared set.
**Prior phase**: Phase 170 (feature; estate consolidation: `ledger_migrate` absorbed with house discipline; closed GH #252 + umbrella #247 -- all six perspective-reset recommendations shipped, Phases 164-170).
**Prior phase**: Phase 169 (feature; evidence reconstruction: `evidence_bundle` 8-signal reconstruction + SCHEMA_REGISTRY freeze rule; closed GH #251).
**Prior phase**: Phase 168 (feature; risk-tiered gate depth: declared artifact set + guarded L1-hotfix audit-skip, fail-closed at four seams; closed GH #248).
**Prior phase**: Phase 167 (feature; dry-run rehearsal on every mutating surface + `session_tool`; closed GH #250 -- autonomous-QA cluster complete).
**Prior phase**: Phase 166 (feature; SG closure enforcement: `closure_enforcer` contract + schema invariant + `sg_closure_lint` corpus worklist; closed GH #249).
**Prior phase**: Phase 165 (feature; autonomous QA nightly: `status_json` aggregate + `nightly-health.yml` with automatic issue lifecycle; closed GH #240).
**Prior phase**: Phase 164 (feature; generate-not-assert: `seal_artifacts` regenerates README badges + SYSTEM_STATE header; the 13-test live-equality class retired).
**Prior phase**: Phase 163 (hotfix; release-pipeline integrity: PyPI publish gated on CI success for the tagged SHA via `release_ci_gate` in both build and publish jobs).
...
|   |-- skills/                           29 SKILL.md across 4 categories
|   |   |-- governance/                   5: qor-audit, qor-substantiate, qor-validate,
|   |   |                                    qor-shadow-process, qor-process-review-cycle
...
3. ✅ Module docstring cites Phase 57 + LLM07 framework gap; contributor origin attribution moves to CHANGELOG per Phase 53/54/55/56 docstring discipline.
...
**Decision**: Phase 57 sealed at v0.43.0. PR #12 superseded by this seal; close after merge with link to Entry #190. The sibling repository's hook consumer package (their B24 PR) can register under `qor_logic.events.gate_written` and observe governance writes without filesystem polling.
...
**Doctrine extension**: `qor/references/doctrine-shadow-genome-countermeasures.md` gains an `SG-CICoverageDrift-A` entry with pattern (plans never reconciled against actual workflows; latent CI failures invisible until integration PR), originating incident (a sibling consumer workspace's 2026-05-22 10-phase stack; `Architecture Guard`), and countermeasure (Phase 89 lint at Step 0.6; WARN-only; plan-side exemption block; self-application test as the deterministic shipping-correctness anchor).
...
**Doctrine extension**: `qor/references/doctrine-shadow-genome-countermeasures.md` gains an `SG-SilentSkipMisconfig-A` entry with pattern (silent-SKIP cascade indistinguishable from legitimate non-Python case), originating recurrence (a sibling consumer workspace per GH #79; project-memory rule encoded silent-skip), and countermeasure (Phase 90 C+D combination; cross-skill test enforces forward-only structural discipline).
...
Feature closing the consumer-blocker half of GH #85 via Option D from the issue's four-option menu. Phase 76 (v0.52.0) introduced forward-only detection/prevention of the concurrent-write race via content-addressable Entry IDs + `check_previous_hash_uniqueness`, but explicitly deferred reconciliation of pre-V1 duplicate entries to a V2 follow-on. Consumer workspaces with the documented residual (e.g., an external QA exemplar's app per GH #85: entries #16a/b, #17a/b, #18a/b sharing previous_hashes from concurrent analytics-376 / live-335 / migration-332 sessions, plus an Entry #20 downstream chain-hash mismatch) could not ship a clean `qor-logic verify-ledger` result on their main ledger and could not pre-commit-hook the chain validator — a usability/release-gate blocker.
...
**New `SG-DoDImplicit-A` doctrine entry** in `qor/references/doctrine-shadow-genome-countermeasures.md`: pattern (implicit DoD; ceremonial gates pass while substantive behavior is unverified), originating recurrence (a sibling consumer workspace per GH #86), countermeasure (Phase 92 V1 structural check), forbidden interpretation (WARN-only V1 contract is the adoption ramp, not permission to ignore D4). Cross-references SG-HalfSealedClaim-A (Phase 75) and SG-DocSurfaceUncovered-A (Phase 58) — same root family: ceremonial gates pass while substantive verification is absent.
...
**New `SG-GrepShapedRunclaim-A` doctrine entry**: catalogs the pattern (recon grades on grep-shaped evidence without proving runtime contract), a sibling consumer workspace's Phase 371 originating recurrence (persona IPC envelope graded HIGH on grep evidence; implementation surfaced runtime import failures, missing modules, syntax errors in tests, zero non-test importers), the V1 detector, and a V2-reserved enforcement clause that Phase 99 will fill in `/qor-audit` Step 3 Infrastructure Alignment Pass (binding-VETO surface).
