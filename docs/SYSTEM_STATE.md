...
**Snapshot**: 2026-06-09
**Chain Status**: ACTIVE. Phase 153 (hotfix, v0.109.1) decomposed `ledger_hash.verify()` behavior-preservingly; Phase 154 (hotfix, v0.109.2) clarified the `seed --target` footgun (closed GH #219); this entry (Phase 155, hotfix, v0.109.3) makes `verify()` FAIL a modern (>= #123) entry that lacks canonical hash markup, closing GAP-GOV-09.
**Phase**: Phase 155 (hotfix, GAP-GOV-09). `ledger_hash.verify()` no longer silently skips a markup-less entry numbered at/after the markup-required cutoff (123) -- it is now a FAIL -- while the 32 historical pre-convention entries (<= #122) stay grandfathered. Grounded in the real data (the skipped set is exactly #1-11 + #68-122; every entry above #122 already has markup, so the modern convention is uniform and no per-type logic is needed). The real chain still verifies clean; the floor only catches a future modern entry written without its hashes. Earlier context: Phase 154 (hotfix, GH #219) added clarifying help to `seed --target` (a destination DIRECTORY, not an artifact name) + the install/uninstall/init `--target` family. The remaining GH #210 (Sprint A) items -- GAP-GOV-05 (self-asserted provenance; needs a non-forgeable signal) and GAP-GOV-03 (TOCTOU) -- are design-heavy and await scoped decisions. **Recent arc (Phases 146-153, all published to PyPI through v0.109.x):** FEATURE_INDEX backfill (#146); a 6-agent production-gap red-team audit (Entry #353) -> Sprint C session_id path-safety + citation accuracy (#147), Sprint B compliance-enforce conveyance + py.typed (#148, GH #211), README currency from a /qor-document review (#149), Sprint A content_hash<->plan binding + provenance/completeness hardening (#150) + dead-hasher removal (#151) + verify() decompose (#153); plus the Shadow Genome producers (#152, GH #213) and Claude Fable 5 (Mythos-class) model-adaptive comms research (Entry #354, deferred). Open backlog: #209 (audit umbrella) + #210 (Sprint A design-heavy remainder); #211/#212/#213/#219 closed; Fable 5 feature deferred. Full per-phase history is authoritative in `docs/META_LEDGER.md` (363 entries; latest Entry #363 -- Phase 155 -- v0.109.3 -- chain `<sealed below>`); SYSTEM_STATE keeps the current-state header, physical map, and a condensed recent-phase bridge rather than restating every phase. The pre-1.0 line is current: the half-measure-closures cluster (GH #147 + #148-#165) is fully closed with real enforcers, and the two largest governance skills remain under the 40 KB skill-size budget (Phases 135/136).
