...
**Snapshot**: 2026-07-04
**Chain Status**: ACTIVE. Phase 168 (feature, v0.116.0) made gate depth scale with declared risk; Phase 169 (feature, v0.117.0) inverted the evidence posture (reconstruction over ceremony + schema freeze); this entry (Phase 170, feature, v0.118.0) closes the perspective-reset umbrella's final item: the estate's proven ledger-migration tool joins the dist. With it, ALL SIX recommendations from research entry #378 are shipped (Phases 164-170, v0.112.0-v0.118.0, one day). (PyPI is a continuous 0.109.0->0.111.0 line, + 0.111.1 through 0.117.0 in flight pending `pypi` approvals, as of 2026-07-04.)
**Phase**: Phase 170 (feature; estate consolidation; research entry #402, closes GH #252 with a recorded premise correction). New `qor/scripts/ledger_migrate.py` (183 lines): normalizes the three legacy hash-markup formats (inline backticks; fenced blocks; session-seal labels) to the canonical form the verifier parses -- never in place (`--output` required, same-path rejected), `--dry-run` rehearsal, idempotent on canonical input, hashes preserved verbatim (markup moves, math does not), dual-formula mismatches and partial/no-hash entries reported with an honest exit 1. Acceptance proven through the REAL verifier (synthetic legacy ledger: 0 -> 3/3 chain-hash-verified). Live rehearsal on our own ledger (dry-run, nothing written): 403 entries, 269 clean, 112 partial, 22 no-hash, 0 mismatches -- the operator's exact worklist should they choose to apply it. Estate dispositions recorded: public-repo mirroring is design-blocked upstream on five operator decisions; the SQLite adapter remains bicameral-mcp's maintained product.
