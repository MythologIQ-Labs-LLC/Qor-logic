...
**Phase**: Phase 277 (hotfix; gate_provenance pair coherence; v0.171.1). `latest_seal_hashes` promised the hash pair of the last ledger entry carrying both canonical markers and took two independent whole-file last matches instead. Replaying the ledger at each historical state, the two came from different entries at 187 of 640, spanning entries #127 to #631, and CI HMACs that pair on every push -- attestations binding entries that never coexisted, not failing, merely meaningless. It now reads one entry, which makes the pair coherent by construction and the value form irrelevant.
