...
**Phase**: Phase 241 (feature; portable governance engine boundary; v0.159.0; GH #381). Defines the base/downstream authority boundary: canonical Qor-logic owns portable governance semantics -- lifecycle, authority, evidence meaning, gate semantics, portable repository contracts, host adaptation, deterministic mechanisms. Downstream enterprise/platform layers produce evidence and project conclusions into enforcement substrates, but may only strengthen, never redefine, what counts as valid governance evidence, and `indeterminate` never reads as satisfied. Authorities: `docs/ADR_PORTABLE_GOVERNANCE_ENGINE_BOUNDARY.md` + `qor/references/downstream-enforcement-boundary.md`, both governance-index registered, protected by 11 boundary regression tests including an ordered authority-direction block assertion and a forge-SDK import scan over executable code.

The revalidation tribunal treated the branch's carried iteration-3 audit artifacts as untrusted (evidence is revision-sensitive after the rebase): the independent reviewer PASSed the content but blocked the seal on the stale artifacts, which were removed in favor of this promotion's own provenance-bearing ceremony; the extension reference relocated out of `qor/platform/` (execution-context adaptation only) with the housing rationale stated in the ADR.
