...
**Snapshot**: 2026-06-09
**Chain Status**: ACTIVE. Phase 152 (feature, v0.109.0) added the Shadow Genome trust/federation/maturity producers (closed GH #213); Phase 153 (hotfix, v0.109.1) decomposed `ledger_hash.verify()` behavior-preservingly; this entry (Phase 154, hotfix, v0.109.2) adds clarifying help to the `seed --target` footgun, closing GH #219.
**Phase**: Phase 154 (hotfix, GH #219). `qor-logic seed --target` now carries clarifying help -- it is a destination workspace DIRECTORY (default: cwd), not an artifact name, so `seed --target FOO` no longer reads as "seed the FOO artifact" (it scaffolds a fresh workspace under ./FOO/); the sibling `install`/`uninstall`/`init` `--target` args gained help too. The issue's secondary exit-1-with-success-output was verified already-resolved in current code; the optional warn-on-nested-workspace is deferred. Earlier context: Phase 153 (hotfix, GAP-CQ-02) decomposed `ledger_hash.verify()` (the production-gap audit's largest-function finding) into a thin orchestrator + `_resolve_recorded` / `_classify_entry` helpers (behavior-preserving; 59-assertion net + 4 helper tests). The remaining GH #210 (Sprint A) items -- GAP-GOV-09 (skipped-entries; needs entry-type classification), GAP-GOV-05 (self-asserted provenance; needs a non-forgeable signal), GAP-GOV-03 (TOCTOU) -- are design-heavy and await scoped decisions. **Recent arc (Phases 146-153, all published to PyPI through v0.109.x):** FEATURE_INDEX backfill (#146); a 6-agent production-gap red-team audit (Entry #353) -> Sprint C session_id path-safety + citation accuracy (#147), Sprint B compliance-enforce conveyance + py.typed (#148, GH #211), README currency from a /qor-document review (#149), Sprint A content_hash<->plan binding + provenance/completeness hardening (#150) + dead-hasher removal (#151) + verify() decompose (#153); plus the Shadow Genome producers (#152, GH #213) and Claude Fable 5 (Mythos-class) model-adaptive comms research (Entry #354, deferred). Open backlog: #209 (audit umbrella) + #210 (Sprint A design-heavy remainder); #211/#212/#213/#219 closed; Fable 5 feature deferred. Full per-phase history is authoritative in `docs/META_LEDGER.md` (362 entries; latest Entry #362 -- Phase 154 -- v0.109.2 -- chain `<sealed below>`); SYSTEM_STATE keeps the current-state header, physical map, and a condensed recent-phase bridge rather than restating every phase. The pre-1.0 line is current: the half-measure-closures cluster (GH #147 + #148-#165) is fully closed with real enforcers, and the two largest governance skills remain under the 40 KB skill-size budget (Phases 135/136).
