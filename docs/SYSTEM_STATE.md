...
**Phase**: Phase 244 (feature; /qor-harden implementation-quality capability; v0.157.0; GH #388). Adds `/qor-harden` as a standalone, environment-neutral implementation-quality review/repair skill: scopes focused/changeset/component/comprehensive, dispositions review/repair, nine canonical IQ dimensions (IQ-COMPLETE, IQ-CORRECT, IQ-TRUST, IQ-CONTEXT, IQ-COMPLEX, IQ-RESOURCE, IQ-CONTRACT, IQ-MAINTAIN, IQ-OBSERVE), abstention as a valid successful outcome, and explicit authority boundaries to `/qor-debug`, `/qor-refactor`, `/qor-plan`, `/qor-remediate`, and `/qor-substantiate`. The canonical protocol lives in `qor/references/implementation-quality-sweep.md`; the invariants unified into `qor/references/doctrine-code-quality.md` (a duplicate standalone doctrine was removed); lifecycle skills reuse the taxonomy through bounded profiles rather than nested `/qor-harden` invocations, encoded in the delegation table's new shared-protocol-reuse section.

The tribunal ran two iterations. Iteration 1 (VETO, independent reviewer): the two selection surfaces -- the SKILL frontmatter description and the qor-help catalog row -- enumerated what read as the complete dimension list while omitting IQ-CONTEXT; and the plan's declared glossary term was never registered while the gate artifact's non-canonical `terms_introduced` key made the doc-integrity glossary check inspect an empty list and pass vacuously (the GH #365/#366 vacuous-gate shape reached through a key-name mismatch). Iteration 2 closed both with a derived-set binding test that regenerates the canonical nine from the sweep's own headings, a registered glossary entry, a canonical-key gate artifact, and a negative probe proving the glossary gate rejects unregistered terms.
