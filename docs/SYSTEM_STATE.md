...
**Phase**: Phase 245 (feature; /qor-refactor modernization; v0.162.0; GH #392 Tranche A). `/qor-refactor` becomes a context-aware, behavior-preserving simplification pass: four declared scope modes (`changeset` recommended post-implementation, `focused`, `component`, `explicit`) with no silent widening; the seven-question Simplification Test gates every proposed change; the post-refactor completion gate blocks on the bad outcomes (contract weakened YES, scope exceeded YES, behavior preserved NO/INCONCLUSIVE); `NO REFACTOR REQUIRED` is a documented success; Section 4 threshold breaches trigger examination through the Simplification Test rather than forced decomposition; JS/TS assumptions are removed or demoted to labeled non-normative examples; and the `/qor-harden` authority boundary (confirmed IQ-COMPLEX/IQ-CONTEXT/IQ-MAINTAIN repairs route here per the canonical sweep) is bound by test.

The promotion tribunal's independent reviewer VETOed on a logically inverted completion gate -- shipped into all six compiled variants and invisible to the 11/11 presence-shaped suite -- and on three unconditional decomposition imperatives contradicting the document's own rule. Closed with polarity and threshold-routing property tests; the reviewer re-verified at source and across every variant. The presence-vs-behavior test-design pattern has now driven reviewer VETOes at Phases 244, 243, 239, and 245; a /qor-process-review-cycle pass over prose-contract testing discipline is the standing recommendation.
