...
**Snapshot**: 2026-06-10
**Chain Status**: ACTIVE. Phase 154 (hotfix, v0.109.2) clarified the `seed --target` footgun (closed GH #219); Phase 155 (hotfix, v0.109.3) made `verify()` FAIL a modern (>= #123) entry that lacks canonical hash markup (GAP-GOV-09); this entry (Phase 156, hotfix, v0.109.4) re-verifies the COMMITTED seal's content_hash<->plan binding in CI (GAP-GOV-03) and, in doing so, fixed a real CRLF fragility in the GOV-01 binding.
**Phase**: Phase 156 (hotfix, GAP-GOV-03). `seal_entry_check --auto` (new `check_latest`, deriving the phase from the latest entry) re-verifies the committed seal's GOV-01 content_hash<->plan binding; a CI step runs it on the committed `META_LEDGER.md` so the binding is re-checked on the committed bytes (ci.yml already re-verified the chain). Running `--auto` against the live ledger surfaced a genuine defect: `ledger_hash.content_hash` was sensitive to git's CRLF conversion -- the seal-time digest is computed on the LF working copy but git rewrites the committed plan to CRLF, so the recompute disagreed. Root-cause fix: `content_hash` now LF-normalizes before hashing (a no-op for LF files, so existing recorded hashes are unchanged); Phase 150's GOV-01 self-test only passed because it ran on the pre-commit LF file, and this phase makes GOV-01 actually hold on committed bytes. The remaining GH #210 (Sprint A) item is GAP-GOV-05 (self-asserted provenance; needs a non-forgeable signal). **Recent arc (Phases 146-156, all published to PyPI through v0.109.x):** FEATURE_INDEX backfill (#146); a 6-agent production-gap red-team audit (Entry #353) -> Sprint C session_id path-safety + citation accuracy (#147), Sprint B compliance-enforce conveyance + py.typed (#148, GH #211), README currency from a /qor-document review (#149), Sprint A content_hash<->plan binding + provenance/completeness hardening (#150) + dead-hasher removal (#151) + verify() decompose (#153) + markup-required floor (#155, GAP-GOV-09) + committed-seal re-verify & CRLF-invariance (#156, GAP-GOV-03); plus the Shadow Genome producers (#152, GH #213) and Claude Fable 5 (Mythos-class) model-adaptive comms research (Entry #354, deferred). Open backlog: #209 (audit umbrella) + #210 (Sprint A: only GAP-GOV-05 remains); #211/#212/#213/#219 closed; Fable 5 feature deferred. Full per-phase history is authoritative in `docs/META_LEDGER.md` (364 entries; latest Entry #364 -- Phase 156 -- v0.109.4 -- chain `<sealed below>`); SYSTEM_STATE keeps the current-state header, physical map, and a condensed recent-phase bridge rather than restating every phase. The pre-1.0 line is current: the half-measure-closures cluster (GH #147 + #148-#165) is fully closed with real enforcers, and the two largest governance skills remain under the 40 KB skill-size budget (Phases 135/136).
