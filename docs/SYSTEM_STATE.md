...
**Phase**: Phase 296 (feature; governed re-attestation of sealed intent-lock evidence; v0.175.0). The publication-boundary doctrine applies retroactively, but CI's `intent_lock_committed` binds a sealed session's plan and plan snapshot to its lock record, so a sealed plan carrying an outside-repository term could not be corrected and the identity-aware seal gate blocked every phase. A walked session's plan hash can now be superseded by an append-only, ledger-committed chain of `<session>.reattest-<k>.json` records; the audit side never changes; the two affected sealed plans are remediated. The lock record itself remains unbound, as before, and the doctrine says so.

**Prior phase**: Phase 291 (hotfix; recompose reconcile.py dialect accessor fix on current main; v0.174.2; GH #477). `reconcile.py`'s `detect_residual` and `_recorded_chain_hash` indexed `ledger_dialect`'s owned `_HASH_VALUE` capture-group layout directly (`match.group(1) or match.group(2)`), silently dropping the third recognized hash-value form -- a bare 64-hex digest alone on its own line. Both now call the published `ledger_dialect.hash_value` accessor, matching `ledger_hash.py`'s existing behavior at its own equivalent call sites. Consolidates two independent current-base recompositions created after Phase 290 superseded historical PR #493 (this branch's own code+tests plus this session's governance evidence) rather than leaving a duplicate lane.

**Prior phase**: Phase 290 (feature; a verifier reports what it examined, and examining nothing is not a pass; v0.174.0). Three gates reported success over a scope that was empty, partial, or supplied by the party they audit, and nothing in the result said so.
...
`ledger_commitment.stale_commitments([])` returned `[]`, indistinguishable from a thorough pass; its checked set is the implement artifact's `files_touched`, and 105 of 204 sealed phases named no plan or brief at all. The install-sync gate compared 32 SKILL.md files and not 26 under `references/` -- 55 percent of the installed markdown surface, silently -- and now compares 58.

Phase 219 established this property for the publication-boundary lint, where `BoundaryResult` carries the scope that produced its findings. It was never generalised, and three open defects were instances of the gap. The doctrine now states it as a general rule.

The labelling pass that found these proposed a six-issue cluster under one root cause; research decomposed it to three sharing a mechanism and three not, before a plan was written against the larger claim. The repaired lint then flagged two stale citations in the plan that repaired it, because the fix moved the lines it cited.
