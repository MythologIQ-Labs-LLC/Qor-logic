...
**Snapshot**: 2026-06-10
**Chain Status**: ACTIVE. Phase 157 (hotfix, v0.109.5) closed a CRLF fragility class for the second seal-relevant file hasher; Phase 158 (feature, v0.110.0) closed the LAST Sprint A item -- non-forgeable gate-artifact provenance (GAP-GOV-05) -- and the GH #210 umbrella; this entry (Phase 159, hotfix, v0.110.1) fixes GH #223 so a downstream `plan-<slug>.md` filename no longer hard-blocks the seal-entry consistency check.
**Phase**: Phase 159 (hotfix; closes GH #223). `seal_entry_check` (substantiate Step 7.7, `|| ABORT`) hard-failed (`rc=1`) on any plan filename not matching the qor-internal `plan-qor-phase<N>-<slug>.md` pattern, blocking `/qor-substantiate` for downstream workspaces (e.g. FailSafe's `plan-<slug>.md`) despite a cryptographically valid seal. The filename only ever supplied the phase NUMBER for the consistency check, so `_main` now routes a non-conforming `--plan` through the existing `check_latest` path (the `--auto` re-verify): it re-derives the phase from the latest ledger entry and runs the identical GOV-01 `content_hash`<->cited-plan binding, emitting a WARN instead of `rc=1`. Not a fail-open relaxation -- a real inconsistency still FAILs (dedicated test). 3 behavioral tests; hotfix; no new deps.
