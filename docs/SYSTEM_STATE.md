...
**Snapshot**: 2026-06-09
**Chain Status**: ACTIVE. Phase 150 (hotfix, v0.108.2) bound the seal's `content_hash` to the plan + gated the provenance-bypass + content-validated completeness; Phase 151 (hotfix, v0.108.3) deleted the dead placeholder session-seal hasher; this entry (Phase 152, feature, v0.109.0) adds the Shadow Genome trust/federation/maturity producer surfaces, closing the downstream FailSafe blocker (GH #213).
**Phase**: Phase 152 (feature, GH #213). The Shadow Genome graph (`qor.scripts.shadow_genome_graph`) now emits the trust-transition / federation-peer / failure-maturity surfaces the FailSafe dashboard renders, under an emitter-API + derive model (qor owns the schema + recorders and derives maturity; the consumer feeds trust/federation): `record_trust_transition`, `set_federation_peer`, `annotate_failure_maturity` + `derive_maturity_stage`; `to_dict` gains `trust_transitions` + `federation_peers` + a per-failure `maturity` field (`nodes`/`edges` unchanged). The #139 "declined -- no consumer" doctrine scope decision is reversed (consumer = FailSafe #196). Open backlog: #209 (umbrella) + #210 (Sprint A; enforcement core + dead-hasher removal done; remainder GAP-CQ-02 decompose `verify()` / GAP-GOV-09 / GAP-GOV-05 / GAP-GOV-03); #211/#212/#213 closed. Earlier context: Phase 151 (hotfix, GAP-GOV-02). Deleted the dead `qor/scripts/calculate-session-seal.py` (a non-importable placeholder hasher with a literal `PREVIOUS_LEDGER_HASH`) and re-pointed the `/qor-substantiate` skill at the real seal helpers. Sprint A remainder: GAP-CQ-02 (decompose `verify()`), GAP-GOV-09 (skipped-entries), GAP-GOV-05 (self-asserted provenance), GAP-GOV-03 (TOCTOU). Earlier context: Phase 150 (hotfix, audit Sprint A integrity binding enforcement core), whose `seal_entry_check` recomputes SHA256 of the plan named in the latest entry's `**Plan**:` field and fails the seal on mismatch or a missing plan (GAP-GOV-01: the recorded `content_hash` is no longer an unverified free field; forward-only -- only the just-sealed entry is recomputed, the 357 prior entries grandfathered). `QOR_GATE_PROVENANCE_OPTIONAL` is honored only under pytest (`_pytest_active`), so a non-test process cannot disable the gate-artifact provenance binding (GAP-GOV-04). `gate_chain_completeness` validates each artifact via `validate_one` (content + schema), not just existence (GAP-GOV-14). Deferred to follow-on phases: GAP-GOV-02 (delete dead `calculate-session-seal.py`), GAP-CQ-02 (decompose `verify()`), GAP-GOV-09 (skipped-entries -- needs entry-type classification), GAP-GOV-05 (self-asserted provenance), GAP-GOV-03 (TOCTOU). Production-gap audit follow-ons: GH #209 (umbrella) + #210 (Sprint A, this entry closes the enforcement core) / #211 (Sprint B, closed) / #212 (Sprint C, closed); Claude Fable 5 (Mythos-class) model-adaptive comms researched (Entry #354) and deferred. Earlier context: Phase 149 (hotfix, README currency). A `/qor-document` thorough review of the two Qor-authored READMEs found `qor/reliability/README.md` fully current and three approved `README.md` edits: the `kilo-code` host-layout row pointed installers at a non-existent `./.kilo-code/` (corrected to `./.kilo/` per `qor/hosts.py`); the CLI Reference now links `qor/references/downstream-enforcement-sdk.md` and notes the Phase-148 `compliance enforce` verdict/status semantics; and a `docs/FEATURE_INDEX.md` row was added to the documentation index. Doc-only; no code change. Earlier context: Phase 148 (feature, audit Sprint B conveyance correctness). `compliance enforce --engagement ci|seal` no longer returns an empty PASS: ci runs `prompt-injection`, seal runs `governance-index` + `gate-chain-completeness` (with a `requires` disclosed-skip if the consumer lacks the artifact), and the two non-CLI controls (`ai-provenance`, `dependency-review`) are surfaced as `disclosed` with a reason. `ControlResult` gains `status` (pass/fail/skip/disclosed) and `Verdict` gains `status` (enforced/failed/no_op); conformance now guards any control carrying a runner; `qor/py.typed` ships (typed SDK) and `compliance/*.json` + `py.typed` are guarded in the packaging test. GAP-TEST-10 (cwd-coupled doctrine tests) folded in; GAP-SEC-01 (conformance importlib allowlist) deferred. Production-gap audit follow-ons: GH #209 (umbrella) + #210 (Sprint A integrity binding, open) / #211 (Sprint B, this entry) / #212 (Sprint C, closed); Claude Fable 5 (Mythos-class) model-adaptive comms researched (Entry #354) and deferred. Full per-phase history is authoritative in `docs/META_LEDGER.md` (360 entries; latest Entry #360 -- Phase 152 -- v0.109.0 -- chain `<sealed below>`); SYSTEM_STATE keeps the current-state header, physical map, and a condensed recent-phase bridge rather than restating every phase. The pre-1.0 line is current: the half-measure-closures cluster (GH #147 + #148-#165) is fully closed with real enforcers, and the two largest governance skills remain under the 40 KB skill-size budget (Phases 135/136).
