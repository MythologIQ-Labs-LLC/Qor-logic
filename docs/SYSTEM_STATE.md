...
**Snapshot**: 2026-07-04
**Chain Status**: ACTIVE. Phase 164 (feature, v0.112.0) made seal presentation artifacts generated-not-asserted; Phase 165 (feature, v0.113.0) added the autonomous QA nightly (aggregate status JSON + scheduled self-check with automatic issue lifecycle; first live run green); this entry (Phase 166, feature, v0.114.0) makes SG closure require an executable enforcer. (PyPI is a continuous 0.109.0->0.111.0 line, + 0.111.1/0.112.0/0.113.0 in flight pending `pypi` approvals, as of 2026-07-04.)
**Phase**: Phase 166 (feature; SG closure enforcement; research entry #386, closes GH #249). A Shadow Genome pattern can no longer close on prose alone -- the finding that prose codification went 0-for-4 against SG-038 recurrence is now structural. `mark_addressed` stage 2 requires a `closure_enforcer` validated fail-closed against exactly four forms (existing `tests/test_*.py` path; importable `qor.scripts.*`/`qor.reliability.*` module; `/qor-<skill> Step N` gate reference; `cannot-automate: <justification >= 50 chars>`), raising `ClosureEnforcerError` without mutation and recording the enforcer on the event; `shadow_event.schema.json` gained the matching if-then invariant (a `remediated` closure requires the field, >= 8 chars). New WARN-only `qor/scripts/sg_closure_lint` (61 lines) walks the 40-entry countermeasure doctrine at every audit (Step 0.6 ladder) -- 10 entries currently cite no enforcer, the standing backfill worklist. All caller sites updated per the audit's SG-AffectedFilesContract-A finding (5 remediate tests, both SKILL.md code examples, doctrine section 10.1 rewritten to the 5-step contract).
