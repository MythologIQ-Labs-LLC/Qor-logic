...
**Phase**: Phase 239 (feature; governed promotion of the /qor-roadmap P1 pilot; v0.161.0; GH #373). Imports the admitted Phase 238 prototype (commit 31243902) under the plan's fail-closed promotion rule: versioned append-only Roadmap events under confined `.qor/roadmaps/<id>/events.jsonl` storage with atomic replace; pure reduction rejecting unsupported versions, malformed history, unknown references, duplicate identities, sequence gaps, cycles, illegal authority, and (iteration 2) re-resolution of an already-resolved node; `fact`/`decision`/`prerequisite` nodes only; evidence-backed facts, authority-gated decisions; complete actionable frontier; fail-closed named-scope handoff ending at `/qor-plan`; delegation-first `/qor-roadmap` skill (Skills badge 32; FEATURE_INDEX FX027, renumbered from the prototype's FX026 by base motion).

The promotion tribunal's independent reviewer verified exact contract fidelity on every clause and VETOed on the one the phase exists to exclude: de-facto decision supersession reachable end-to-end from the shipped CLI. Closed with a status guard mirroring the retired-space handler, red-proved, re-verified at source by the reviewer; duplicate-identity and sequence-gap clauses gained behavioral coverage. Evaluation of Roadmap against the current baseline remains GH #373's next task; supersession, ranking, concurrency, tracker projection, and enterprise consumption require separate admission.
