...
**Scope**: How the change affects (1) qor-logic systems independently and (2) qor-logic in concert with the downstream consumer application

---

## Executive Summary

GH #196 asks to extend the existing FEATURE_INDEX verification pass so that, *when a repo's `FEATURE_INDEX.md` declares a `Surface` column*, each non-`n/a` feature must also carry a `surface` value (and resolve its release). Grounded against source, the change is a **schema-optional, additive, WARN-first capability extension** — not a behavior change to any existing gate. The blast radius on qor-logic *itself* is effectively zero today: qor-logic's own repo has **no `FEATURE_INDEX.md`** (confirmed: `ls docs/FEATURE_INDEX.md` → absent; `governance-health` → `MISSING`), so it disclosed-skips the entire pass and a fortiori the new sub-check. The change is therefore a pure **framework capability** that only bites in consumer repos (a sibling governance repository) that adopt both the index and the column. The data is the sibling governance repository's (an external repository's issue); the gate is qor-logic's. Two latent issues surfaced: (a) a classification drift between `governance_health` (treats `FEATURE_INDEX.md` as *required*) and `doctrine-feature-inventory.md` (declares it *optional*), and (b) a V1→V2 sequencing contract that must hold across the two repos to avoid breaking the sibling governance repository's seal.
...
### Downstream consumer interaction effects

- **Split ownership, classic enforce-here / supply-there.** The Surface *data* is the sibling governance repository's (an external repository's issue adds the column to consumer-side `FEATURE_INDEX.md`); the *gate* is qor-logic's (`/qor-substantiate` is a SHIELD skill owned here, consumed by a sibling governance repository). No runtime API coupling exists — the gate reads the sibling governance repository's markdown; no a sibling governance repository code calls qor-logic. The contract is purely the FEATURE_INDEX markdown format plus the gate's expectations.
- **Enforcement only bites when both land.** Until qor-logic ships the check, the sibling governance repository's `Surface` column is ungoverned free-text the parser tolerates as an extra trailing column. The benign window is consumer-ships-first (WARN-only anyway). The dangerous window is the reverse at V2: if qor-logic flips **fail-closed before a sibling governance repository populates every non-`n/a` row**, the sibling governance repository's seal breaks. Hence WARN-first is the correct rollout, and **a sibling governance repository must reach full surface-column coverage before qor-logic promotes V2** — the same half-measure-vs-V2 ladder discipline already in play across the cluster, here intentional (consumer adoption runway), not a deferred half-measure.
- **The net-new governed datum is narrow.** The motivating use-case is the sibling governance repository's Development Tracker projecting per-surface lifecycle progress from FEATURE_INDEX + META_LEDGER; the "got it wrong twice" pain was that surface↔feature↔release was never governed at seal time. Of the issue's two asks, the **release** half is largely already resolvable (verification status + ledger linkage); the genuinely new governed field is **surface**. So V1's enforcement reduces to "surface-tag presence on non-`n/a` rows."
- **Version contract.** The check ships in a qor-logic release; a new gate capability is `change_class: minor` per the governance flow. The consumer must upgrade/pin to that qor-logic version to receive enforcement. No force-coupling — adoption is pull-based.
...
6. **(Medium, cross-repo) Gate V2 promotion on a sibling governance repository coverage.** Do not remove the `|| true` until the external repository's issue reports 100% surface-tag coverage on non-`n/a` rows; otherwise the sibling governance repository's seal breaks.
...
Added to `qor/references/doctrine-feature-inventory.md`: a "Surface column (optional V2 extension, GH #196)" subsection capturing the schema-optional/WARN-first/fail-closed-on-a sibling governance repository-coverage contract, so the doctrine reflects the planned column ahead of implementation.
