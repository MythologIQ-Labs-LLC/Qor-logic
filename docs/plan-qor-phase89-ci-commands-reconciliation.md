...
- `python -m qor.reliability.intent_lock_committed --phase-min 231` â€” Phase 233 ci.yml `gate-chain-completeness` job: committed intent-lock evidence gate (GH #352; forward-maintenance; command introduced after Phase 89 seal).
...
- `python -m qor.scripts.status_json --self-test` — Phase 165 nightly-health.yml step: the aggregate checker validates its own aggregation logic before any verdict is trusted (forward-maintenance; command introduced after Phase 89 seal).
- `python -m qor.scripts.status_json --repo-root .` — Phase 165 nightly-health.yml step: aggregate governance-health verdict with final-line JSON contract, feeding the automatic issue lifecycle (forward-maintenance; command introduced after Phase 89 seal).
- `python -m qor.scripts.publication_boundary_lint --repo-root .` — Phase 208 ci.yml `gate-chain-completeness` job step: fail-closed publication-boundary enforcement over the tracked surface, now that the doctrine's two standing exceptions are expressible (forward-maintenance; command introduced after Phase 89 seal).
- `python -m qor.scripts.github_surface --repo "${{ github.repository }}"` — Phase 211 nightly-health.yml step: publication-boundary scan of issue and pull-request titles, bodies, and comments, which the tracked-file lint cannot reach (forward-maintenance; command introduced after Phase 89 seal).
- `python -m ruff check qor/ tests/` — Phase 213 ci.yml `gate-chain-completeness` job step: pyflakes-only lint, adopted narrowly and landed at zero with no baseline (forward-maintenance; command introduced after Phase 89 seal).
