...
`git show f3e069b:qor/scripts/check_shadow_threshold.py | grep -n 'def _signature\|details.get("gate")\|hashlib.sha256' -> observed:
...
`git show f3e069b:qor/scripts/check_shadow_threshold.py | grep -n 'def sweep\|elif e\["severity"\] >= 3\|aged_entry_id' -> observed:
...
`git show f3e069b:qor/scripts/check_shadow_threshold.py | grep -n 'elif e\["severity"\] >= 3\|new_event = {' -> observed:
63:        elif e["severity"] >= 3 and e["id"] not in existing_escalations:
64:            new_event = {`
...
93-117 of `f3e069b`'s copy) unchanged, renamed. This is the signature an
