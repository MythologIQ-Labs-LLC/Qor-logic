...
### Fixed
- **Phase 248 (hotfix; plan-terms key canonicalization)**: closes GH #394, where the plan-artifact glossary check inspected nothing and reported success. `qor/gates/schema/plan.schema.json` defined the canonical field as `terms` and `doc_integrity.run_all_checks_from_plan` read exactly that key, but `/qor-plan`'s dialogue, its markdown template, and two doctrines all instructed a different name, `terms_introduced`. Under `additionalProperties: true` a plan authored exactly per those instructions validated cleanly while the glossary check received an empty declared-terms list, so an unregistered term passed documentation integrity unexamined. The schema now carries a `not: {required: [terms_introduced]}` rule so the retired alias fails validation instead of passing silently, and every authoring surface (`qor/skills/sdlc/qor-plan/SKILL.md`, its `references/step-extensions.md`, `doctrine-documentation-integrity.md`, `doctrine-ideation-readiness.md`) names the canonical `terms`; all six host variants are recompiled. `doc_integrity.py` is unchanged, its read path having always been correct. The guard is red-proved by execution at seal time: removing the schema rule fails the negative probe, restoring it passes, and the schema is byte-identical afterward. Closing the alias route does not close the omission route -- `terms` remains absent from schema `required`, so an artifact declaring neither key still evaluates to an empty list; that residual is declared as a plan non-goal and filed as GH #414 rather than left silent.
- **Phase 248 scope item 4 (release blocker found at seal time)**: a schema tightening must not retroactively invalidate already-sealed evidence. `gate_chain_completeness` validates every sealed phase's gate artifacts against the *current* schema, so the new prohibition invalidated eight artifacts across sealed phases 187, 191 and 192 and the fail-closed gate exited 1 -- meaning no seal in this repository could ever complete again, including the one shipping the fix. `validate_gate_artifact.validate_one` gains a keyword-only `sealed_history` flag that drops only the schema's top-level `not` clause, and both sealed-history readers opt in: `gate_chain_completeness` (fails closed at seal time) and `evidence_bundle` (fails open into an evidence packet). `required`, `type`, `properties` and `$ref` still apply, so GAP-GOV-14 is unchanged and an empty, malformed or required-field-missing artifact still fails. Enforcement of new prohibitions stays at authoring time in `gate_chain.write_gate_artifact`, which is forward-only by construction. The eight historical artifacts are left byte-identical, so every `.provenance` sidecar's `payload_sha256` still binds -- migrating the key would have traded a schema failure for an evidence-integrity failure.
...
### Added
- **Phase 194 (feature; unified governance-path + ledger-dialect handling)**: closes GH #282, a class of health-clean-but-gate-aborts contradictions where a downstream gate re-derived a governance path or ledger dialect with a stricter local assumption than the authoritative governance-health pre-gate. Three shared modules now own what four gates previously hardcoded. `qor/scripts/governance_paths.py` resolves the architecture authority (from the `GOVERNANCE_INDEX` registration, so a repository whose sole registered authority is `docs/ARCHITECTURE_PLAN.md` passes system-tier topology without a duplicate `docs/architecture.md`; the legacy literal keeps precedence when present) and validates active plan paths (a registered `plan-<slug>.md` is admitted, while traversal / outside-root / unsupported-extension / unregistered paths are rejected before any read). `qor/scripts/ledger_dialect.py` is the single source for the accepted hash-markup dialect -- inline-backtick, `= <hex>`, and fenced bare-hex -- plus the separate `**Phase**:` line and the one historical compatibility boundary (entry 123), consumed by `ledger_hash`, `seal_entry_check`, and (transitively) `governance_health` so all three agree; no rejection is relaxed (content/chain mismatch, malformed hashes, post-boundary duplicate previous hashes, and tampering still fail) and no adopter history is rewritten. `qor/scripts/version_applicability.py` moves the version downgrade guard before audit PASS (a release-class plan whose target could not exceed the current tag is caught at audit, not after implementation) and adds the explicit non-release `change_class: governance` classification that plan, audit, and substantiate interpret identically (version-not-applicable: no bump, no tag).

### Changed
- `doc_integrity.check_topology` resolves the system-tier architecture slot through the shared resolver instead of the hardcoded `docs/architecture.md`.
- `prompt_injection_canaries` validates the active plan path through the shared resolver instead of a `docs/plan-qor-phase*.md`-only allowlist.
- `plan.schema.json` `change_class` enum admits `governance` (non-release).
...
Consolidated release: eighteen sealed governed phases (174-191) shipped as one
version. Per-phase seals and internal version history are recorded in
`docs/META_LEDGER.md` entries #410-#485.
